...
extension STFilterLooksModel {
    ...
    func getSourceFromPath(bundleName: String, bmpType: STEffectsType) -> [STFilterLooksModel] {
        ...
        let strLocalBundlePath = CustomPodspecBundle.bundle.path(forResource: bundleName, ofType: "bundle")
        ...
        for i in 0...(arrFiles?.count ?? 1)-1 {
            ...
            if strFileName.hasSuffix(".zip") {
                ...
                if imageThumb == nil {
                    imageThumb = UIImage.eponymUp(name: "st_none")
                }
    ...
    func getFilterBundleSource(bundleName: NSString) -> [STFilterLooksModel] {
        let arrFiles = try? FileManager.default.contentsOfDirectory(atPath: UserMacroDefine.overfill())
        ...
        for i in 0...(arrFiles?.count ?? 1)-1 {
            ...
            if strFileName.hasSuffix("model") {
                ...
                if orignName.count > 0 {
                    sourceDict[orignName] = NSString.path(withComponents: [UserMacroDefine.overfill(), strFileName ])
                }
            }
        }
