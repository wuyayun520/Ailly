...
class STFilterListItemCell: UICollectionViewCell {
    ...
    lazy var nameLab: UILabel = {
        let lb = UILabel()
        lb.font = UIFont.outsideAllSize(fontSize: 12)
        lb.textColor = .white
    ...
    lazy var borderView: UIView = {
        ...
        view.layer.borderColor = UIColor.plaster().cgColor
...
extension STFilterListItemCell {
    ...
    func setCellModel(model: STFilterLooksModel) {
        self.iconView.image = UIImage.eponymUp(name: model.filterNormalImageName)
        self.nameLab.text = model.strName
        self.borderView.isHidden = !model.isSelected
        if model.isSelected {
            self.nameLab.textColor = UIColor.plaster()
        }else {
