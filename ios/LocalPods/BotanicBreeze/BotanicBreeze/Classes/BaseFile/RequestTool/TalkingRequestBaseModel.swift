...
/*: "http://stat. :*/
fileprivate let str_errorValue:String = "http://false property"
fileprivate let str_viewData:String = "let name let var nonestat."

/*: .com" :*/
fileprivate let str_userData:String = "number cookie self.com"

/*: "http://" :*/
fileprivate let str_addLiveValue:[Character] = ["h","t","t","p",":","/","/"]

/*: "https://" :*/
fileprivate let str_moreListName:String = "print importhttps://"

/*: "http://m. :*/
fileprivate let str_documentName:String = "httbottom"

/*: "http:// :*/
fileprivate let str_userValue:String = "price cover else sexhttp:/"
fileprivate let str_viewName:[Character] = ["/"]

/*: "-1" :*/
fileprivate let str_topValue:[Character] = ["-","1"]

/*: "3,90" :*/
fileprivate let str_errorData:[Character] = ["3",",","9","0"]

/*: "1990-01-01" :*/
fileprivate let str_onData:[Character] = ["1","9","9","0","-","0","1","-","0","1"]

/*: "MM-dd-yyyy" :*/
fileprivate let str_labText:[Character] = ["M","M","-","d","d","-","y","y","y","y"]

/*: "id" :*/
fileprivate let str_replyValue:[Character] = ["i","d"]
...
class WithTitleThen: NSObject {
    ...
    var requestType: ArtisticStyleRequestType?
    ...
    override init() {
        ...
        self.requestServer = SendUpReactiveCompatible.share.serverUrlStr
...
struct AddModelType: HandyJSON {
    ...
    var CSConfig = PortraitCameraFreeModelType() // 私聊官方客服提示消息模型
//    var jumpRandomVideo: Bool = false       // true表示可以跳转到随机视频匹配
    ...
    var reportDomain: String {
        ...
        if statDomain.isEmptyString {
            ...
            return (String(str_errorValue.prefix(7)) + String(str_viewData.suffix(5))) + "\(kLet_deviceName)" + (String(str_userData.suffix(4)))
            //: } else {
        } else {
            //: if !statDomain.hasPrefix("http://") && !statDomain.hasPrefix("https://") {
            if !statDomain.hasPrefix((String(str_addLiveValue))) && !statDomain.hasPrefix((String(str_moreListName.suffix(8)))) {
                //: return "http://".appending(statDomain)
                return (String(str_addLiveValue)).appending(statDomain)
                //: }else {
            } else {
                //: return statDomain
                return statDomain
    ...
    var urlH5Domain: String {
        ...
        if urlDomain.isEmpty {
            ...
            return (str_documentName.replacingOccurrences(of: "bottom", with: "p") + "://m.") + "\(kLet_deviceName)" + (String(str_userData.suffix(4)))
            //: } else {
        } else {
            //: return "http://\(urlDomain)"
            return (String(str_userValue.suffix(6)) + String(str_viewName)) + "\(urlDomain)"
        }
    }
...
public class ShadowTransformable: NSObject, HandyJSON {
    ...
    var chatPriceSettings: [DataHandyJSON] = [] // 收费设置
    //: var videoPriceSettings: Array<ChatPriceModel> = []
    var videoPriceSettings: [DataHandyJSON] = [] // 视频收费设置
    //: var voicePriceSettings: Array<ChatPriceModel> = []
    var voicePriceSettings: [DataHandyJSON] = [] // 语音收费设置
    ...
    var vipSetting = [TheoryMeasurable]() // vip皮肤
    ...
    var tabPageBanner = [DealTransformable]() // V1.6.4 一级页面广告
    ...
    var intimateLimit = ImpendentEventComputerSimulationTransformable() // 1.7.6 各种操作的亲密度限制
...
@objcMembers public class RunUserModel: NSObject, HandyJSON {
    //: required public override init() {
    override public required init() {}
    ...
    var interest: [OfHandyJSON]?
    //: var aboutMe: Array<UserSeleteTagModel>?
    var aboutMe: [OfHandyJSON]?
    //: var partner: String?
    var partner: String?
    //: var full: Int?
    var full: Int?
    ...
    var callPopupSetting = (String(str_errorData)) // 表示来电弹窗配置，如”3,90”表示3s后弹第一次，后续90s，返回”0,0”表示不弹窗
    //: var callTabSwitch = 0
    ...
    var residualInfo = [ViewModelType]() // 套餐数量

    //: func updateResidualNum(type: Int, num: Int) {
    func turnTo(type: Int, num: Int) {
    ...
    public lazy var age: String = {
        ...
        let birthdayDate = NSDate.maturityVideo(self.birthday ?? (String(str_onData)), dateFormat: (String(str_labText)))
        //: guard let currentYear = NSDate.getCurrentDateComponents().year else { return "0" }
        guard let currentYear = NSDate.labOf().year else { return "0" }
...
struct CustomModelType: HandyJSON {
    //: var id: String?
    var id: String?
    //: var url: String?
    var url: String?
...
public struct OfHandyJSON: HandyJSON, Equatable {
    //: var tag_id: String?
    var tag_id: String?
    //: var tag_name = ""
    var tag_name = ""
    ...
    func tagObject(compareTo: OfHandyJSON) -> Bool {
        //: return
        return
...
struct TagModel: HandyJSON {
    ...
    var aboutMe: [NameTagMeasurable]!
    //: var interest: Array<UserTagTypeModel>!
    var interest: [NameTagMeasurable]!
}

//: struct UserTagTypeModel: HandyJSON {
struct NameTagMeasurable: HandyJSON {
    //: var name = ""
    var name = ""
    //: var name_ar = ""
    var name_ar = ""
    ...
    var list: [OfHandyJSON]!
}

/// 官方客服提示消息模型
//: struct CSConfigModel: HandyJSON {
struct PortraitCameraFreeModelType: HandyJSON {
...
struct TheoryMeasurable: HandyJSON {
...
struct ImpendentEventComputerSimulationTransformable: HandyJSON {
    //: var sendImg = 10
    var sendImg = 10 // 发送图片
    //: var sendVideo = 10
