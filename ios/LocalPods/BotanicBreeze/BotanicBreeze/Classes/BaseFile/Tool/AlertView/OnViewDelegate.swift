...
/*: "cellID" :*/
fileprivate let str_dataValue:[Character] = ["c","e","l","l","I","D"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let str_talkData:[UInt8] = [0xc2,0xc7,0xc2,0xcd,0x81,0xbc,0xc8,0xbd,0xbe,0xcb,0x93,0x82,0x79,0xc1,0xba,0xcc,0x79,0xc7,0xc8,0xcd,0x79,0xbb,0xbe,0xbe,0xc7,0x79,0xc2,0xc6,0xc9,0xc5,0xbe,0xc6,0xbe,0xc7,0xcd,0xbe,0xbd]

fileprivate func cellExecute(add num: UInt8) -> UInt8 {
...
/*: "Video Call" :*/
fileprivate let str_equalValue:[Character] = ["V","i","d","e","o"," ","C","a","l","l"]

/*: "Video Call (%@ Points/Min)" :*/
fileprivate let str_spendValue:String = "Video model lab name"
fileprivate let str_pathText:String = " (online@"
fileprivate let str_fileData:String = "/Min)lab return view"

/*: "Voice Call" :*/
fileprivate let str_normalTitle:[Character] = ["V","o","i","c","e"," ","C","a","l","l"]

/*: "Voice Call (%@ Points/Min)" :*/
fileprivate let str_cellValue:String = "Voicestyle page tip image member"
fileprivate let str_appTitle:String = "l (%@ let make"
fileprivate let str_rankInputAddData:[Character] = ["s","/","M","i","n",")"]

/*: "#F8F8F8" :*/
fileprivate let str_shareText:[Character] = ["#"]
fileprivate let str_actionName:String = "f8f8f8"

/*: "Cancel" :*/
fileprivate let str_rangeName:String = "file color self self equalCancel"

/*: "#F5F5F5" :*/
fileprivate let str_attentionModeTitle:String = "#F5F5F5stack else add number color"

/*: "icon_lounge" :*/
fileprivate let str_callValue:String = "icon_lto false count share number"
...
class OnViewDelegate: UIView {
    //: var munuBlock: MunuBlock?
    var munuBlock: MunuBlock?

    //: private var type: MunuPopType = .Unknown
    private var type: ScalePopType = .Unknown // 当前展示类型
    ...
    var cellTitleColor = UIColor.executeTo()
    /// cell字体大小
    //: var cellTitleFont = UIFont.pingfangFont(type: .Regular, fontSize: 16)
    var cellTitleFont = UIFont.pingfang(type: .Regular, fontSize: 16)
    /// 是否有"Cancel"菜单
    //: var hasCancel = true
    var hasCancel = true
    /// cancel字体颜色
    //: var cancelColor = UIColor.publishView()
    var cancelColor = UIColor.publishView()
    /// cancel字体大小
    //: var cancelTitleFont = UIFont.pingfangFont(type: .Regular, fontSize: 16)
    var cancelTitleFont = UIFont.pingfang(type: .Regular, fontSize: 16)
    /// 标题
    //: var title: String?
    var title: String?
    /// 标题颜色
    //: var titleColor = UIColor.appThemeColor()
    var titleColor = UIColor.plaster()
    /// 标题字体大小
    //: var titleFont = UIFont.pingfangWithDiameter(fontSize: 16)
    var titleFont = UIFont.pingfangWithDiameter(fontSize: 16)

    //: var tableView = UITableView()
    var tableView = UITableView()
    //: let cellID = "cellID"
    let cellID = (String(str_dataValue))
    ...
    fileprivate var footerViewHight: CGFloat {
        ...
        return hasCancel ? (footerViewLineHight + 50 + kLet_nextData) : kLet_nextData
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_talkData.map{cellExecute(add: $0)}, encoding: .utf8)!)
    }

    //: func initwithList(cellTitleList: [String]!) {
    func videoList(cellTitleList: [String]!) {
        // 初始化
    ...
    func canny(type: ScalePopType, videoPrice: String?, voicePrice: String?) {
        //: self.type = type
        self.type = type
        //: if self.type == .Call {
        if self.type == .Call { // 音视频通话
            ...
            if EqualThen.share.appStatus == PhaseOfTheMoonPropertyProtocol.normal.rawValue {
                ...
                if EqualThen.share.loginUserMode.sex == RunReplaceableCollection.male.rawValue {
                    ...
                    let attributes = [.foregroundColor: UIColor.plaster(), .font: UIFont.pingfang(type: .Regular, fontSize: 14)] as [NSAttributedString.Key: Any]
                    //: let VideoCall = "Video Call".localized
                    let VideoCall = (String(str_equalValue)).localized
                    //: let videoStr = "Video Call (%@ Points/Min)".localizedArguments(videoPrice ?? "0")
                    let videoStr = (String(str_spendValue.prefix(6)) + "Call" + str_pathText.replacingOccurrences(of: "online", with: "%") + " Points" + String(str_fileData.prefix(5))).localizedArguments(videoPrice ?? "0")
                    ...
                    let VoiceCall = (String(str_normalTitle)).localized
                    //: let voiceStr = "Voice Call (%@ Points/Min)".localizedArguments(voicePrice ?? "0")
                    let voiceStr = (String(str_cellValue.prefix(5)) + " Cal" + String(str_appTitle.prefix(6)) + "Point" + String(str_rankInputAddData)).localizedArguments(voicePrice ?? "0")
                ...
                self.cellTitleList = [(String(str_equalValue)).localized, (String(str_normalTitle)).localized]
            }
        }
    ...
    public func someItem() {
        ...
        tableView = UITableView(frame: CGRect(x: 0, y: kLet_clickData - tableViewHight, width: kLet_cameraValue, height: tableViewHight), style: .plain)
        ...
        tableView.register(ShowEqualViewCell.self, forCellReuseIdentifier: cellID)
        ...
        if title == nil {
            ...
            tableView.showEqual(width: kLet_cameraValue,
...
extension OnViewDelegate: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        var cell = tableView.dequeueReusableCell(withIdentifier: cellID) as? ShowEqualViewCell
        //: if cell == nil {
        if cell == nil {
            //: cell = TitleCell(style: .default, reuseIdentifier: cellID)
            cell = ShowEqualViewCell(style: .default, reuseIdentifier: cellID)
        }

        //: if EqualThen.share.loginUserMode.sex == Gender.male.rawValue,
        if EqualThen.share.loginUserMode.sex == RunReplaceableCollection.male.rawValue,
           //: EqualThen.share.loginUserMode.loungePlus == true,
           EqualThen.share.loginUserMode.loungePlus == true,
           //: type == .Call {
           type == .Call
        {
        ...
        cell!.titleLab.font = cellTitleFont
        //: if self.type == .Call && EqualThen.share.appStatus == AppSkinStatus.normal.rawValue {
        if self.type == .Call && EqualThen.share.appStatus == PhaseOfTheMoonPropertyProtocol.normal.rawValue {
    ...
    func tableView(_: UITableView, viewForFooterInSection _: Int) -> UIView? {
        ...
        let footerView = UIView(frame: CGRect(x: 0, y: 0, width: kLet_cameraValue, height: footerViewHight))
        //: let lineView = UIView(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: footerViewLineHight))
        let lineView = UIView(frame: CGRect(x: 0, y: 0, width: kLet_cameraValue, height: footerViewLineHight))
        //: lineView.backgroundColor = UIColor(hex: "#F8F8F8")
        lineView.backgroundColor = UIColor(hex: (String(str_shareText) + str_actionName.uppercased()))
        ...
        btn.frame = CGRect(x: 0, y: footerViewLineHight, width: kLet_cameraValue, height: 50)
        //: footerView.addSubview(btn)
        footerView.addSubview(btn)
        //: btn.setTitle("Cancel".localized, for: .normal)
        btn.setTitle((String(str_rangeName.suffix(6))).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(messageCallDoing), for: .touchUpInside)
        //: return footerView
        return footerView
    }
    ...
    func tableView(_: UITableView, viewForHeaderInSection _: Int) -> UIView? {
        ...
        let headerView = UIView(frame: CGRect(x: 0, y: 0, width: kLet_cameraValue, height: headerViewHight))

        //: let titleLab = UILabel(frame: CGRect(x: 15, y: 0, width: ScreenWidth-30, height: headerViewHight))
        let titleLab = UILabel(frame: CGRect(x: 15, y: 0, width: kLet_cameraValue - 30, height: headerViewHight))
        ...
        let lineView = UIView(frame: CGRect(x: 0, y: headerViewHight - 1, width: kLet_cameraValue, height: 1))
        //: lineView.backgroundColor = UIColor.init(hex: "#F5F5F5")
        lineView.backgroundColor = UIColor(hex: (String(str_attentionModeTitle.prefix(7))))
...
class ShowEqualViewCell: UITableViewCell {
    ...
    lazy var vipIcon: UIImageView = {
        ...
        img.image = UIImage.eponymUp(name: (String(str_callValue.prefix(6)) + "ounge"))
        //: img.isHidden = true
        img.isHidden = true
        //: return img
        return img
    ...
    lazy var lineLab: UILabel = {
        ...
        lab.backgroundColor = UIColor(hex: (String(str_attentionModeTitle.prefix(7))))
        //: return lab
        return lab
        //: }()
    }()
