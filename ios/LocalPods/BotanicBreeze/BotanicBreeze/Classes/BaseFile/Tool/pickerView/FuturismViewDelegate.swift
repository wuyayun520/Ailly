...
/*: "Select a date" :*/
fileprivate let str_nextTitle:String = "Selectequal stop self"
fileprivate let str_kitData:[Character] = [" ","a"," ","d","a","t","e"]

/*: M" :*/
fileprivate let str_resourceValue:String = "action"

/*: D" :*/
fileprivate let str_appValue:String = "send"

/*: Y" :*/
fileprivate let str_partyName:String = "hidden"
...
class FuturismViewDelegate: EmancipatedThen, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    override func giftInit() {
        //: super.initPickView()
        super.giftInit()

        //: titleString = "Select a date".localized
        titleString = (String(str_nextTitle.prefix(6)) + String(str_kitData)).localized
        ...
        clueInDay(year: defaultYear!, month: defaultMonth!, day: defaultDay!)
    }

    //: func setDefaultDate(year: Int, month: Int, day: Int) {
    func clueInDay(year: Int, month: Int, day: Int) {
        //: currentYear = year
        currentYear = year
        //: if year == defaultYear {
        if year == defaultYear {
...
//        }
    ...
    override func observerWith(button: UIButton) {
        //: super.confirmClick(button: button)
        super.observerWith(button: button)
        //: refreshPickViewData()
        collectionViewPickApplication()

        //: if let day = selectDay {
        if let day = selectDay {
            //: delegate?.pickDateView(year: selectYear!, month: selectMonth!, day: day)
            delegate?.viewDay(year: selectYear!, month: selectMonth!, day: day)
            //: } else {
        } else {
            //: delegate?.pickDateView(year: selectYear!, month: selectMonth!, day: 0)
            delegate?.viewDay(year: selectYear!, month: selectMonth!, day: 0)
        }
    }
        ...
        if isShowDay {
            ...
            if component == 0 {
                ...
                if currentYear == yearSum + minShowYear - 1 {
                    ...
                    return hinduCalendarMonth(year: year, month: month)
                }
                //: } else {
            } else {
                //: return self.yearSum
    ...
    func hinduCalendarMonth(year: Int, month: Int) -> Int {
    ...
    func collectionViewPickApplication() {
