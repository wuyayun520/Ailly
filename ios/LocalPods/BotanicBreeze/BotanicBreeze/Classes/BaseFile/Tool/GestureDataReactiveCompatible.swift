...
/*: "Finish" :*/
fileprivate let str_editData:[Character] = ["F","i","n","i","s","h"]

/*: "Cancel" :*/
fileprivate let str_topName:[Character] = ["C","a","n","c","e","l"]

/*: "btn_photo_choice_pre" :*/
fileprivate let str_shareLeadingTitle:[Character] = ["b","t","n","_","p","h","o","t","o","_","c","h","o","i","c","e","_"]
fileprivate let str_labValue:String = "PRE"

/*: "btn_photo_add_nor" :*/
fileprivate let str_errorText:[Character] = ["b","t","n","_","p","h","o","t","o","_","a","d","d","_","n","o","r"]

/*: "( :*/
fileprivate let str_deadlineName:[Character] = ["("]

/*: )" :*/
fileprivate let str_callRowName:[Character] = [")"]

/*: "/tmp" :*/
fileprivate let str_fillValue:[Character] = ["/","t","m","p"]

/*: "Export session failed" :*/
fileprivate let str_angleValue:String = "Exportcell show label call"
fileprivate let str_willSectionContent:[Character] = ["s","i","o","n"," ","f","a","i","l","e","d"]

/*: "Export canceled" :*/
fileprivate let str_photoText:String = "model object guardExport"
fileprivate let str_modelData:[Character] = ["c","e","l","e","d"]

/*: "Successful!" :*/
fileprivate let str_managerData:String = "user return titleSu"
fileprivate let str_menuContent:String = "ccessful!first will true"

/*: "Unsupported video formats" :*/
fileprivate let str_currentData:String = "up available point self myUnsu"
fileprivate let str_touchIndexCallValue:String = "d vidtransaction container return nearby"
fileprivate let str_aspectName:String = "ormatshare"

/*: "yyyy-MM-dd-HH:mm:ss-SSS" :*/
fileprivate let str_firstData:String = "yyyy-Mtext log extension"
fileprivate let str_pathName:[Character] = ["m","m",":","s","s","-","S","S","S"]

/*: "/tmp/video- :*/
fileprivate let str_liveValue:[Character] = ["/","t","m","p","/","v","i"]
fileprivate let str_withAddValue:String = "deo-temp item"

/*: - :*/
fileprivate let str_tableData:String = "target"

/*: .mp4" :*/
fileprivate let str_textName:String = ".mp4self top"
...
// __CLOSE_PRINT__
//
//  GestureDataReactiveCompatible.swift
//  AbroadTalking
//
...
class GestureDataReactiveCompatible: NSObject {
    ...
    class func extra(allowImgCrop: Bool = false, needCircleCrop: Bool = false, maxCount: Int, allowPhoto: Bool, allowVideo: Bool) -> TZImagePickerController {
...
//        imagePicker?.preferredLanguage = "en"
        ...
        if allowImgCrop {
            ...
            var height = kLet_cameraValue * 488 / 375
            //: if needCircleCrop == true {
            if needCircleCrop == true { // 切圆图
                //: height = ScreenWidth
                height = kLet_cameraValue
            }
            //: imagePicker?.cropRect = CGRect(x: 0, y: (ScreenHeight-height)/2, width: ScreenWidth, height: height)
            imagePicker?.cropRect = CGRect(x: 0, y: (kLet_clickData - height) / 2, width: kLet_cameraValue, height: height)
        }
        //: if allowVideo {
        if allowVideo {
        ...
        imagePicker?.doneBtnTitleStr = (String(str_editData)).localized
        //: imagePicker?.cancelBtnTitleStr = "Cancel".localized
        imagePicker?.cancelBtnTitleStr = (String(str_topName)).localized

        //: imagePicker?.barItemTextFont = UIFont.pingfangFont(type: .Regular, fontSize: 15)
        imagePicker?.barItemTextFont = UIFont.pingfang(type: .Regular, fontSize: 15)
        //: imagePicker?.barItemTextColor = UIColor.appValueColor()
        imagePicker?.barItemTextColor = UIColor.addColor()
        ...
        imagePicker?.naviTitleFont = UIFont.pingfang(type: .Medium, fontSize: 17)
        //: imagePicker?.naviTitleColor = UIColor.appTitleColor()
        imagePicker?.naviTitleColor = UIColor.executeTo()
        ...
        imagePicker?.iconThemeColor = UIColor.plaster()

        //: imagePicker?.allowPickingOriginalPhoto = false
        imagePicker?.allowPickingOriginalPhoto = false
        //: imagePicker?.photoSelImage = UIImage.eponymUp(name: "btn_photo_choice_pre")
        imagePicker?.photoSelImage = UIImage.eponymUp(name: (String(str_shareLeadingTitle) + str_labValue.lowercased()))
        //: imagePicker?.takePictureImage = UIImage.eponymUp(name: "btn_photo_add_nor")
        imagePicker?.takePictureImage = UIImage.eponymUp(name: (String(str_errorText)))
        ...
        imagePicker?.photoPickerPageDidRefreshStateBlock = { (_: UICollectionView?,
                ...
                doneButton?.titleLabel?.font = UIFont.pingfang(type: .Medium, fontSize: 14)
                ...
                if text.count > 0 && Int(text)! > 0 {
                    ...
                    doneButton?.setTitle((String(str_editData)).localized + "(\(text))", for: .normal)
                    //: } else {
                } else {
                    //: doneButton?.setTitle("Finish".localized, for: .normal)
                    doneButton?.setTitle((String(str_editData)).localized, for: .normal)
                }
                //: return
        }
        ...
        imagePicker?.photoPickerPageDidLayoutSubviewsBlock = { (_: UICollectionView?,
                ...
                doneButton?.backgroundColor = UIColor.plaster()
                //: let text = numberLabel?.text ?? "0"
                let text = numberLabel?.text ?? "0"
                //: if text.count > 0 && Int(text)! > 0 {
                if text.count > 0 && Int(text)! > 0 {
        ...
        imagePicker?.photoPreviewPageDidLayoutSubviewsBlock = { (_: UICollectionView?,
                ...
                selectButton?.frame = CGRect(x: kLet_cameraValue - 5 - (selectButton?.frame.size.width)!, y: (selectButton?.frame.origin.y)!, width: (selectButton?.frame.size.width)!, height: (selectButton?.frame.size.height)!)
                //: let text = numberLabel?.text ?? "0"
                let text = numberLabel?.text ?? "0"
                //: if text.count > 0 && Int(text)! > 0 {
                if text.count > 0 && Int(text)! > 0 {
    ...
    class func completionEqual(asset: PHAsset?, completion: @escaping (_ filePath: URL?) -> Void) {
        ...
        PHImageManager.default().requestAVAsset(forVideo: asset!, options: options) { (asset: AVAsset?, _: AVAudioMix?, _) in
            ...
            if asset!.isKind(of: AVURLAsset.self) {
                ...
                if !FileManager.default.fileExists(atPath: NSHomeDirectory().appending((String(str_fillValue)))) {
                    //: do {
                    do {
                        //: try FileManager.default.createDirectory(atPath: NSHomeDirectory().appending("/tmp"), withIntermediateDirectories: true)
                        try FileManager.default.createDirectory(atPath: NSHomeDirectory().appending((String(str_fillValue))), withIntermediateDirectories: true)
                        //: } catch {
                    } catch {}
                }
                ...
                exportSession?.exportAsynchronously(completionHandler: {
                    ...
                    case .failed:
                        ...
                        printLog(message: (String(str_angleValue.prefix(6)) + " ses" + String(str_willSectionContent)))
                    //: case .cancelled:
                    case .cancelled:
                        //: printLog(message: "Export canceled")
                        printLog(message: (String(str_photoText.suffix(6)) + " can" + String(str_modelData)))
                    //: case .completed:
                    case .completed:
                        //: printLog(message: "Successful!")
                        printLog(message: (String(str_managerData.suffix(2)) + String(str_menuContent.prefix(9))))
                        //: DispatchQueue.main.async {
                        DispatchQueue.main.async {
                ...
                printLog(message: (String(str_currentData.suffix(4)) + "pporte" + String(str_touchIndexCallValue.prefix(5)) + "eo f" + str_aspectName.replacingOccurrences(of: "share", with: "s")))
            }
        }
    }
    ...
    private class func getPath() -> String {
        ...
        formater.dateFormat = (String(str_firstData.prefix(6)) + "M-dd-HH:" + String(str_pathName))
        //: let outputPath = NSHomeDirectory().appending("/tmp/video-\(formater.string(from: Date()))-\(Int.random(in: 0...10000000)).mp4")
        let outputPath = NSHomeDirectory().appending((String(str_liveValue) + String(str_withAddValue.prefix(4))) + "\(formater.string(from: Date()))-\(Int.random(in: 0 ... 10_000_000))" + (String(str_textName.prefix(4))))
        //: return outputPath
        return outputPath
    }
}
