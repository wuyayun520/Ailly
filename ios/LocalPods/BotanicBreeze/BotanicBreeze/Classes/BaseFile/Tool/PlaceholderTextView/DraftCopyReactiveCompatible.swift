...
/*: / :*/
fileprivate let str_matchTitle:String = "tag"

/*: "\n" :*/
fileprivate let str_containerContent:String = "\n"
...
// __CLOSE_PRINT__
//
//  DraftCopyReactiveCompatible.swift
//  DraftCopyReactiveCompatible
...
struct RepresentContainerInset {
    //: let top: CGFloat = 5.0
    let top: CGFloat = 5.0
...
class DraftCopyReactiveCompatible: UIView {
    // MARK: - 懒加载属性
    ...
    convenience init(placeholder: String?, placeholderColor: UIColor?, frame: CGRect) {
        ...
        upColor(placeholder: placeholder, placeholderColor: placeholderColor)
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        upColor(placeholder: nil, placeholderColor: nil)
    }
}
...
extension DraftCopyReactiveCompatible {
    /// placeholder Label Setup
    //: private func setup(placeholder: String?, placeholderColor: UIColor?) {
    private func upColor(placeholder: String?, placeholderColor: UIColor?) {
        ...
        plaleLabel.snp.remakeConstraints { make in
            ...
            make.leading.equalTo(ShowLabelOrigin().x)
            //: make.top.equalTo(PlaceholderLabelOrigin().y)
            make.top.equalTo(ShowLabelOrigin().y)
            //: make.trailing.equalTo(-PlaceholderLabelOrigin().x)
            make.trailing.equalTo(-ShowLabelOrigin().x)
        }
        ...
        placeholdertextView.textContainerInset = UIEdgeInsets(top: RepresentContainerInset().top, left: RepresentContainerInset().left, bottom: RepresentContainerInset().bottom, right: RepresentContainerInset().right)
...
extension DraftCopyReactiveCompatible: UITextViewDelegate {
