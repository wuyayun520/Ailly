...
let kLet_pointContent = 3.0

//: class CircleCountdownView: UIView {
class EventNumberdownView: UIView {
    // 圆圈背景色
    //: var roundBgColor = UIColor.appValueColor() {
    var roundBgColor = UIColor.addColor() {
        //: willSet {
        willSet {
    ...
    lazy var timeLab: UILabel = {
        ...
        lab.font = UIFont.pingfang(type: .Semibold, fontSize: 19)
    ...
    private lazy var bPath: UIBezierPath = {
        ...
        let rect = CGRect(x: kLet_pointContent / 2,
                          //: y: CIRCLECOUNTDOWNLINE_W/2,
                          y: kLet_pointContent / 2,
                          //: width: self.frame.width-CIRCLECOUNTDOWNLINE_W,
                          width: self.frame.width - kLet_pointContent,
                          //: height: self.frame.height-CIRCLECOUNTDOWNLINE_W)
                          height: self.frame.height - kLet_pointContent)
    ...
    private lazy var outLayer: CAShapeLayer = {
        ...
        layer.strokeColor = UIColor.addColor().cgColor
        //: layer.lineWidth = CIRCLECOUNTDOWNLINE_W
        layer.lineWidth = kLet_pointContent
...
extension EventNumberdownView {
    /// 创建UI
    //: private func createUI() {
    private func enterAppear() {
    ...
    func leftSlanting(totalTime: Int, currTime: Int) {
