...
fileprivate let str_pauseValue:[UInt8] = [0x7a,0x7d,0x7a,0x67,0x3b,0x70,0x7c,0x77,0x76,0x61,0x29,0x3a,0x33,0x7b,0x72,0x60,0x33,0x7d,0x7c,0x67,0x33,0x71,0x76,0x76,0x7d,0x33,0x7a,0x7e,0x63,0x7f,0x76,0x7e,0x76,0x7d,0x67,0x76,0x77]

private func hiddenGameRaw(model num: UInt8) -> UInt8 {
    return num ^ 19
}

/*: "FFD855" :*/
fileprivate let str_equalBackgroundName:String = "ffd855"
...
class AtView: UIView {
    ...
    var defaultWidth: CGFloat = 0.0 {
        ...
        didSet {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.2) {
                ...
                let minX = self.leftSilder.frame.origin.x + self.defaultWidth - kLet_attributeName
                //: self.rightSlider.frame = CGRect(x: minX,
                self.rightSlider.frame = CGRect(x: minX,
                                                //: y: self.rightSlider.frame.origin.y,
                                                y: self.rightSlider.frame.origin.y,
                                                //: width: kSliderWidth,
                                                width: kLet_attributeName,
                ...
                self.scienceLaboratory()
                //: self.refreshSliderValidScope(startX: self.videoStartX)
                self.permitX(startX: self.videoStartX)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_pauseValue.map{hiddenGameRaw(model: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var indicatorView: UIView = {
        ...
        indicator.backgroundColor = UIColor(hex: (str_equalBackgroundName.uppercased()))
        //    indicator.alpha = 0.5
    ...
    private lazy var leftSilder: UIView = {
        ...
        leftSilder.frame = CGRect(x: kLet_liveShowName, y: 0, width: kLet_attributeName, height: self.frame.size.height)
    ...
    private lazy var rightSlider: UIView = {
        ...
        rightSlider.frame = CGRect(x: self.frame.size.width - kLet_attributeName, y: 0, width: kLet_attributeName, height: self.frame.size.height)
    ...
    private lazy var leftCoverView: UIView = {
        ...
        leftCoverView.backgroundColor = UIColor.showSub(0, 0, 0, 0.6)
        //: leftCoverView.frame = CGRect(x: 0, y: 0, width: LeftMargin, height: self.frame.size.height)
        leftCoverView.frame = CGRect(x: 0, y: 0, width: kLet_liveShowName, height: self.frame.size.height)
    ...
    private lazy var rightCoverView: UIView = {
        ...
        rightCoverView.backgroundColor = UIColor.showSub(0, 0, 0, 0.6)
...
extension AtView {
    ...
    func simulation() {
        ...
        self.leftSilder.frame = CGRect(x: kLet_liveShowName, y: 0, width: kLet_attributeName, height: self.frame.size.height)
        //: self.rightSlider.frame = CGRect(x: self.frame.size.width - kSliderWidth, y: 0, width: kSliderWidth, height: self.frame.size.height)
        self.rightSlider.frame = CGRect(x: self.frame.size.width - kLet_attributeName, y: 0, width: kLet_attributeName, height: self.frame.size.height)
        //: resetIndicator()
        titleIndicator()
    }

    //: func needShowIndicator(isShow: Bool) {
    func sizeShow(isShow: Bool) {
        //: self.indicatorView.isHidden = !isShow
        self.indicatorView.isHidden = !isShow
    }

    //: func refreshSliderValidScope(startX: CGFloat) {
    func permitX(startX: CGFloat) {
        ...
        rightScope = CGVector(dx: leftSilder.frame.minX + minDurationGap - kLet_attributeName,
                              //: dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - kSliderWidth)
                              dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - kLet_attributeName)
    }
}
...
extension AtView {
    ...
    @objc func errorGesture(gesture: UIPanGestureRecognizer) {
        ...
        case .ended:
            ...
            self.permitX(startX: videoStartX)
    ...
    @objc func statesRightsGesture(gesture: UIPanGestureRecognizer) {
        //: let thisTimePoint = gesture.location(in: self)
        let thisTimePoint = gesture.location(in: self)
        //: let state = gesture.state
        let state = gesture.state
...
extension AtView {
    ...
    private func giftTotal() {
        ...
        let leftPanGesture = UIPanGestureRecognizer(target: self, action: #selector(errorGesture(gesture:)))
        ...
        let rightPanGesture = UIPanGestureRecognizer(target: self, action: #selector(statesRightsGesture(gesture:)))
    ...
    func windowBridge(_ xposition: CGFloat) {
    ...
    private func scienceLaboratory() {
