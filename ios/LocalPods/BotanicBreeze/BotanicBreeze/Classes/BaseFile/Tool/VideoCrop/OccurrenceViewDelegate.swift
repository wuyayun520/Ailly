...
fileprivate let str_brideData:[UInt8] = [0x20,0x27,0x20,0x3d,0x61,0x2a,0x26,0x2d,0x2c,0x3b,0x73,0x60,0x69,0x21,0x28,0x3a,0x69,0x27,0x26,0x3d,0x69,0x2b,0x2c,0x2c,0x27,0x69,0x20,0x24,0x39,0x25,0x2c,0x24,0x2c,0x27,0x3d,0x2c,0x2d]

private func viewLabel(skin num: UInt8) -> UInt8 {
    return num ^ 73
}
...
typealias SlidingHandler = (_ direction: ShadowSlideDirection) -> Void
...
class OccurrenceViewDelegate: UIView {
    //: var cropStart: CGFloat {
    var cropStart: CGFloat {
        //: let offset = overlay.startOffset - (LeftMargin - self.contentView.contentOffset.x)
        let offset = overlay.startOffset - (kLet_liveShowName - self.contentView.contentOffset.x)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_brideData.map{viewLabel(skin: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIScrollView = {
        ...
        while time <= self.duration {
            ...
            let image = self.callMentalPicture(point: time)
            //: imv.image = image
            imv.image = image
    ...
    lazy var overlay: AtView = {
        ...
        let overlay = AtView(frame: self.bounds, maxCropLen: maxWidth, minCropLen: minWidth)
...
extension OccurrenceViewDelegate {
    /// 重置位置
    //: func resetPostion() {
    func wordsWithoutCell() {
        ...
        self.overlay.permitX(startX: kLet_liveShowName - x)
    }
    ...
    private func callMentalPicture(point: CGFloat) -> UIImage {
...
extension OccurrenceViewDelegate: UIScrollViewDelegate {
    //: func scrollViewDidScroll(_ scrollView: UIScrollView) {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        //: var offset = scrollView.contentOffset
        var offset = scrollView.contentOffset
        //: let minx = LeftMargin - overlay.startOffset
        let minx = kLet_liveShowName - overlay.startOffset
    ...
    func scrollViewWillBeginDragging(_: UIScrollView) {
        ...
        self.overlay.sizeShow(isShow: false)
    }
    ...
    func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        ...
        self.overlay.sizeShow(isShow: true)
    }
...
extension OccurrenceViewDelegate {
    ...
    private func statusHeadSection() {
        ...
        self.contentView.snp.makeConstraints { make in
            ...
            make.leading.equalToSuperview().offset(kLet_liveShowName)
