...
fileprivate let str_blockTitle:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "btnJQbEAB_etihw_kcab_shadow" :*/
fileprivate let str_tagText:[Character] = ["b","t","n","J","Q"]
fileprivate let str_loadValue:String = "image stylebEAB_et"
fileprivate let str_postPathValue:String = "as needihw_k"
fileprivate let str_lineName:String = "infoadow"

/*: "Edit" :*/
fileprivate let str_indexName:String = "view type view sign atEdit"

/*: "Finish" :*/
fileprivate let str_gestureCurrentValue:String = "Finisview self raw block image"
fileprivate let str_modelName:[Character] = ["h"]

/*: "btnYDo2AS_oediv_play" :*/
fileprivate let str_giftBackName:String = "error equal lab classbtnYDo"
fileprivate let str_sectionStartName:[Character] = ["2","A","S"]
fileprivate let str_mutualValue:[Character] = ["_","o","e","d","i","v","_","p","l","a","y"]

/*: "Cancel" :*/
fileprivate let str_dataTitle:String = "c"
fileprivate let str_cellSourceTitle:String = "anceremove"

/*: "bgQkRTbF_oediv_toast" :*/
fileprivate let str_statusValue:[Character] = ["b"]
fileprivate let str_addLoadData:[Character] = ["g","Q","k","R","T","b","F","_","o","e","d","i","v","_","t","o","a","s","t"]

/*: "The video length should be %@-%@ seconds. Please intercept it" :*/
fileprivate let str_picValue:[UInt8] = [0x73,0x87,0x84,0x3f,0x95,0x88,0x83,0x84,0x8e,0x3f,0x8b,0x84,0x8d,0x86,0x93,0x87,0x3f,0x92,0x87,0x8e,0x94,0x8b,0x83,0x3f,0x81,0x84,0x3f,0x44,0x5f,0x4c,0x44,0x5f,0x3f,0x92,0x84,0x82,0x8e,0x8d,0x83,0x92,0x4d,0x3f,0x6f,0x8b,0x84,0x80,0x92,0x84,0x3f,0x88,0x8d,0x93,0x84,0x91,0x82,0x84,0x8f,0x93,0x3f,0x88,0x93]

fileprivate func pathInput(block num: UInt8) -> UInt8 {
...
// __CLOSE_PRINT__
//
//  EditVisualCommunicationViewController.swift
//  AbroadTalking
//
...
typealias VideoEditBlock = (_ videoInfo: TotaleractionHandyJSON?) -> Void

//: let kMinVideoDuration: CGFloat = 5.0
let kLet_marginName: CGFloat = 5.0

//: class TalkingVideoEditorController: TalkingBaseViewController {
class EditVisualCommunicationViewController: SeamViewController {
    ...
    private var slider: OccurrenceViewDelegate?
    //: private var videoInfo = TalkingVideoInfoModel()
    private var videoInfo = TotaleractionHandyJSON() // 编辑结果模型
    //: private var currStyle = TalkingVideoEditorStyle.normal
    private var currStyle = SoundEditorStyle.normal // 当前样式
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if DeviceSocketDelegate.shared.isTalking == true { // 正在音视频通话，静音播放
    ...
    init(style: SoundEditorStyle = .normal, maxDuration: CGFloat, videoURL: URL, completionHandler: @escaping VideoEditBlock) {
        ...
        imgWidth = (kLet_cameraValue - 2 * padding) / (framesInScreen * 1.0)
        //: imgHeight = 50
        imgHeight = 50
        //: duration = 0
        duration = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_blockTitle.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var backBtn: UIButton = {
        ...
        let v = UIButton(type: .custom)
        //: v.setImage(UIImage.eponymUp(name: "btnJQbEAB_etihw_kcab_shadow"), for: .normal)
        v.setImage(UIImage.eponymUp(name: (String(str_tagText) + String(str_loadValue.suffix(7)) + String(str_postPathValue.suffix(5)) + "cab_s" + str_lineName.replacingOccurrences(of: "info", with: "h"))), for: .normal)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = FileReactiveCompatible.shared.direction == .rightToLeft ? .right : .left
        //: v.addTarget(self, action: #selector(backAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(finder), for: .touchUpInside)
    ...
    private lazy var editBtn: UIButton = {
        ...
        v.setTitle((String(str_indexName.suffix(4))).localized, for: .normal)
        //: v.setTitleColor(UIColor.appThemeColor(), for: .normal)
        v.setTitleColor(UIColor.plaster(), for: .normal)
        //: v.titleLabel?.font = UIFont.outsideAllSize(fontSize: 14)
        v.titleLabel?.font = UIFont.outsideAllSize(fontSize: 14)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = FileReactiveCompatible.shared.direction == .rightToLeft ? .right : .left
        //: v.isHidden = true
...
//        v.addTarget(self, action: #selector(editAction(sender:)), for: .touchUpInside)
    ...
    private lazy var bottomDoneBtn: UIButton = {
        ...
        v.setTitle((String(str_gestureCurrentValue.prefix(5)) + String(str_modelName)).localized, for: .normal)
        //: v.setTitleColor(UIColor.white, for: .normal)
        v.setTitleColor(UIColor.white, for: .normal)
        //: v.titleLabel?.font = UIFont.pingfangWithDiameter(fontSize: 14)
        v.titleLabel?.font = UIFont.pingfangWithDiameter(fontSize: 14)
        ...
        v.backgroundColor = UIColor.plaster()
        //: v.addTarget(self, action: #selector(bottomDoneAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(transgression), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: return v
        return v
    ...
    private lazy var playBtn: UIButton = {
        //: let v = UIButton(type: .custom)
        let v = UIButton(type: .custom)
        //: v.setImage(UIImage.eponymUp(name: "btnYDo2AS_oediv_play"), for: .normal)
        v.setImage(UIImage.eponymUp(name: (String(str_giftBackName.suffix(6)) + String(str_sectionStartName) + String(str_mutualValue))), for: .normal)
        //: v.addTarget(self, action: #selector(playAction(sender:)), for: .touchUpInside)
        v.addTarget(self, action: #selector(outShould(sender:)), for: .touchUpInside)
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        //: return v
        return v
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        v.setTitle((str_dataTitle.uppercased() + str_cellSourceTitle.replacingOccurrences(of: "remove", with: "l")).localized, for: .normal)
    ...
    private lazy var confirmBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(successGuide), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: v.snp.makeConstraints { make in
        v.snp.makeConstraints { make in
    ...
    private lazy var toastImageView: UIImageView = {
        //: let v = UIImageView(image: UIImage.eponymUp(name: "bgQkRTbF_oediv_toast"))
        let v = UIImageView(image: UIImage.eponymUp(name: (String(str_statusValue) + String(str_addLoadData))))
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        ...
        lab.text = String(bytes: str_picValue.map{pathInput(block: $0)}, encoding: .utf8)!.localizedArguments(minDuration, Int(maxduration))
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.numberOfLines = 0
        lab.numberOfLines = 0
    ...
    lazy var useVideoView: IndexOffThen = {
        //: let v = TalkingSendIntimateInfoView(frame: CGRect.zero, type: .intimateVideo)
        let v = IndexOffThen(frame: CGRect.zero, type: .intimateVideo)
        //: v.isHidden = true
        v.isHidden = true
        //: self.view.addSubview(v)
        self.view.addSubview(v)
...
extension EditVisualCommunicationViewController {
    /// 返回按钮事件
    ...
    private func changeFormAndAnimated(animated: Bool = true) {
    ...
    @objc func isTask() {
        ...
        self.previewLayer?.frame = CGRect(x: 26, y: 26, width: kLet_cameraValue - 2 * 26, height: kLet_clickData - 46 - 127 - kLet_nextData)
        //: imgWidth = ScreenWidth / framesInScreen
        imgWidth = kLet_cameraValue / framesInScreen

        //: if slider == nil {
        if slider == nil {
            //: let frame = CGRect(x: 0, y: ScreenHeight, width: ScreenWidth, height: CGFloat(imgHeight))
            let frame = CGRect(x: 0, y: kLet_clickData, width: kLet_cameraValue, height: CGFloat(imgHeight))
            // 创建预览区域
            ...
            slider = OccurrenceViewDelegate(frame: frame, url: self.videoURL!, imgWidth: imgWidth, maxDuration: maxduration, minVideoDuration: kLet_marginName)
            ...
            slider?.snp.makeConstraints { make in
                ...
                make.bottom.equalToSuperview().offset(-60 - kLet_nextData)
...
//                self.playing = false
            ...
            slider?.slidingBlock = { [weak self] (direction: ShadowSlideDirection) in
                //: guard let self = self else { return }
                guard let self = self else { return }
...
//                        self.playBtn.isHidden = true
    ...
    @objc func transgression() {
        ...
        if self.videoEditHandler != nil {
            ...
            ListReactiveCompatible.shared.allHandler(videoInfo: videoInfo, containerView: self.view, saveDocuments: true) { [weak self] outputPath, coverPath in
                //: guard let self = self else { return }
                guard let self = self else { return }
                //: self.popCurrentViewController()
                self.changeFormAndAnimated()
    ...
    @objc func outShould(sender: UIButton) {
...
extension EditVisualCommunicationViewController {
    ...
    private func dartPlayer() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(doingWith), name: NSNotification.Name.AVPlayerItemDidPlayToEndTime, object: item)
        ...
        self.playerTimer = self.player?.addPeriodicTimeObserver(forInterval: step, queue: DispatchQueue.main, using: { [weak self] time in
            ...
            if self.slider != nil, self.slider!.isHidden == false {
                ...
                self.slider?.overlay.windowBridge(xposition)
            }
            //: }) as AnyObject?
        }) as AnyObject?
    ...
    private func startInstance() {
        ...
        self.botBar.snp.makeConstraints { make in
            ...
            make.height.equalTo(48 + kLet_nextData)
        }
    ...
    private func equalOn() {
        // 监听应用进入后台
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(didEnterBackground),
                                               selector: #selector(listEqual),
