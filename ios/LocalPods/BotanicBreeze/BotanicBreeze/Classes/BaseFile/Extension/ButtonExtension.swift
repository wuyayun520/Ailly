...
/*: "recharge" :*/
fileprivate let str_scaleValue:String = "rechartext"
fileprivate let str_observeValue:[Character] = ["e"]

/*: "recharge_ :*/
fileprivate let str_toName:String = "recvideoarge"
fileprivate let str_giftValue:String = "_"

/*: ".png" :*/
fileprivate let str_verticalTitle:String = "mode leading.png"

/*: "_ :*/
fileprivate let str_sData:String = "_"

/*: .png" :*/
fileprivate let str_photographValue:String = ".pngleading action model"

/*: ".jpg" :*/
fileprivate let str_formatModelViewData:[Character] = [".","j","p","g"]

/*: .jpg" :*/
fileprivate let str_toLimitPlusValue:String = ".jpgsucceed model"
...
enum RawDataImageAlignment: NSInteger {
    //: case left = 0
    case left = 0
    //: case top
    case top
...
class ProgressTalkingButton: UIButton {
    ...
    var imageAlignment: RawDataImageAlignment = .left
...
extension UIButton {
    ...
    func enableLine(_ interval: CGFloat) {
        //: verticalAlignImageAndTitle(interval: interval, topOffset: 0)
        indexPic(interval: interval, topOffset: 0)
    }
    ...
    func indexPic(interval: CGFloat, topOffset: CGFloat) {
    ...
    func outsideDrunk(interval: CGFloat, imageSize: CGSize, topOffset: CGFloat) {
...
extension UIButton {
    ...
    func likeness(color: UIColor, forState: UIControl.State) {
    ...
    func dataBack(urlStr: String,
                  //: placeImg: UIImage? = UIImage.placeImgSquare(),
                  placeImg: UIImage? = UIImage.becomeSection(),
                  ...
                  loadFinish: ((_ isSucceed: Bool) -> Void)? = nil)
    {
        ...
        let imgUrl = urlStr.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: kLet_deviceName)
    ...
    func pushStr(urlStr: String) {
        //: guard !urlStr.isEmptyString else { return }
        guard !urlStr.isEmptyString else { return }
        //: var newUrl = ""
        var newUrl = ""
        //: if urlStr.contains("recharge") {
        if urlStr.contains((str_scaleValue.replacingOccurrences(of: "text", with: "g") + String(str_observeValue))) {
            //: newUrl = urlStr.replacingOccurrences(of: "recharge", with: "recharge_\(PackageID)")
            newUrl = urlStr.replacingOccurrences(of: (str_scaleValue.replacingOccurrences(of: "text", with: "g") + String(str_observeValue)), with: (str_toName.replacingOccurrences(of: "video", with: "h") + str_giftValue.capitalized) + "\(kLet_playPageValue)")
            //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
            self.dataBack(urlStr: newUrl, placeImg: nil)
            //: return
            return
                //: } else if urlStr.contains(".png") {
        } else if urlStr.contains((String(str_verticalTitle.suffix(4)))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".png", with: "_\(PackageID).png")
            newUrl = urlStr.replacingOccurrences(of: (String(str_verticalTitle.suffix(4))), with: "_\(kLet_playPageValue)" + (String(str_photographValue.prefix(4))))

            //: } else if urlStr.contains(".jpg") {
        } else if urlStr.contains((String(str_formatModelViewData))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".jpg", with: "_\(PackageID).jpg")
            newUrl = urlStr.replacingOccurrences(of: (String(str_formatModelViewData)), with: "_\(kLet_playPageValue)" + (String(str_toLimitPlusValue.prefix(4))))
        }
        //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
        self.dataBack(urlStr: newUrl, placeImg: nil)
    }
