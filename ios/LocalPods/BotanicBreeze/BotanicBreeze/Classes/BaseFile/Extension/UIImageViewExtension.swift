...
/*: "add_header_cookies" :*/
fileprivate let str_labelName:String = "add_app premium kit data"
fileprivate let str_modelValue:String = "_cookextension day size view"
fileprivate let str_gestureFemaleData:[Character] = ["i","e","s"]
...
public extension UIImageView {
    ...
    @objc func direct(urlStr: String,
    ...
    internal func lineBorder(urlStr: String,
                             //: placeImg: UIImage = UIImage.placeImgSquare(),
                             placeImg: UIImage = UIImage.becomeSection(),
    ...
    @objc func ofBy(urlStr: String) {
        //: let imgUrl = urlStr.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let imgUrl = urlStr.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: kLet_deviceName)
        //: self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.giftPlaceHolderImage())
        self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.betweenEqual())
    }
    ...
    @objc func videoView(urlStr: String, completion: ((_ image: UIImage?) -> Void)? = nil) {
        // 允许请求携带cookie
        ...
        if var urlComponents = URLComponents(string: urlStr),
           ...
           let currURL = URL(string: SendUpReactiveCompatible.share.serverUrlStr)
        {
        ...
        let downloader = ImageDownloader(name: (String(str_labelName.prefix(4)) + "header" + String(str_modelValue.prefix(5)) + String(str_gestureFemaleData)))
        ...
        self.kf.setImage(with: URL(string: newImgUrl), placeholder: UIImage.becomeSection(), options: optionsInfo) { result in
            //: if completion != nil {
            if completion != nil {
                //: let image = try? result.get().image
                let image = try? result.get().image
