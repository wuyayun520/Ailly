...
public class ThreadPlayerSafeArray<Element> {
...
public extension ThreadPlayerSafeArray {
    //: var first: Element? {
    var first: Element? {
        //: var result: Element?
        var result: Element?
...
public extension ThreadPlayerSafeArray {
    ...
    func filter(isIncluded: @escaping (Element)->Bool) ->ThreadPlayerSafeArray<Element> {
        //: pthread_rwlock_rdlock(&lock)
        pthread_rwlock_rdlock(&lock)
        ...
        return ThreadPlayerSafeArray(array: result)
    }
    ...
    func sorted(by areInIncreasingOrder: (Element, Element)->Bool)->ThreadPlayerSafeArray<Element> {
        //: pthread_rwlock_rdlock(&lock)
        pthread_rwlock_rdlock(&lock)
        //: let result = array.sorted(by: areInIncreasingOrder)
        let result = array.sorted(by: areInIncreasingOrder)
        //: let newArray = ThreadSafeArray<Element>(array: result)
        let newArray = ThreadPlayerSafeArray<Element>(array: result)
        //: pthread_rwlock_unlock(&lock)
        pthread_rwlock_unlock(&lock)
        //: return newArray
        return newArray
...
public extension ThreadPlayerSafeArray where Element: Equatable {
...
public extension ThreadPlayerSafeArray {
    ...
    static func +=(left: inout ThreadPlayerSafeArray, right: Element) {
        //: left.append(right)
        left.append(right)
    }
