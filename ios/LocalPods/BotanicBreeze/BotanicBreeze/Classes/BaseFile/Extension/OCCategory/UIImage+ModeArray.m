...
#import "UIImage+ModeArray.h"

//: @implementation UIImage (Compress)
@implementation UIImage (ModeArray)
...
- (UIImage *)custom {
    ...
    return [UIImage imageWithData:[self key]];
}
...
- (UIImage *)gift:(Boolean)isSession {
    //: CGSize size = [self wxImageSize:isSession];
    CGSize size = [self scaleValue:isSession];
    //: UIImage *reImage = [self resizedImage:size];
    UIImage *reImage = [self resizedMentalImage:size];
...
- (CGSize)scaleValue:(Boolean)isSession {
...
+ (NSData *)quote:(UIImage *)image quantityRelation:(NSInteger)fileSize{
    ...
    while (outIMGData.length>(fileSize*1024)) {
        ...
        thumIMG = [thumIMG format:scalesize keyScale:NO];

        //: outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
        outIMGData = [thumIMG handle:(fileSize*1024)];
    }
...
- (UIImage*)quoteSize:(CGSize)targetSize{
...
- (UIImage *)format:(CGSize)targetSize keyScale:(BOOL)isScale{
...
- (NSData *)handle:(NSInteger)fileSize{
...
- (UIImage *)resizedMentalImage:(CGSize)newSize {
