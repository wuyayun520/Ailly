...
fileprivate let str_offLabelData:[UInt8] = [0xe1,0xe6,0xe1,0xfc,0xa0,0xeb,0xe7,0xec,0xed,0xfa,0xb2,0xa1,0xa8,0xe0,0xe9,0xfb,0xa8,0xe6,0xe7,0xfc,0xa8,0xea,0xed,0xed,0xe6,0xa8,0xe1,0xe5,0xf8,0xe4,0xed,0xe5,0xed,0xe6,0xfc,0xed,0xec]

/*: "CustomMenuCell" :*/
fileprivate let str_shareValue:[Character] = ["D","r","o","p","M","e","m","b","e","r","M","e","n","u","C","e","l","l"]

/*: "roomId" :*/
fileprivate let str_sectionContent:String = "roomIdimage var equal let model"

/*: "type" :*/
fileprivate let str_viewName:[Character] = ["t","y","p","e"]

/*: "page" :*/
fileprivate let str_onTitle:[Character] = ["p","a","g","e"]

/*: "icon_chats_pcr" :*/
fileprivate let str_myTitle:[Character] = ["i","c","o","n","_","c","h","a","t"]
fileprivate let str_sizeValue:String = "s_pcrfor gift in line"

/*: "All Numbers" :*/
fileprivate let str_colorData:String = "All one object lab"
fileprivate let str_upValue:[Character] = ["N","u","m","b","e","r","s"]

/*: "all" :*/
fileprivate let str_componentName:String = "aon"

/*: "Recent Gifts" :*/
fileprivate let str_barInputName:[Character] = ["R","e","c","e","n","t"," ","G","i","f","t","s"]
...
protocol IndexCameraViewDelegate {
    //: func  didClickSelectedRow(model: TalkingChatRoomMemberModel)
    func computerSimulation(model: GrantUserMeasurable)
}

//: class DropDownMemberMenuView: UIView {
class MakeMenuView: UIView {
    //: var pageIndex = 0
    var pageIndex = 0
    //: var roomID = ""
    var roomID = ""
    //: var menuDelegate: DropDownMemberMenuViewDelegate?
    var menuDelegate: IndexCameraViewDelegate?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_offLabelData.map{$0^136}, encoding: .utf8)!)
    }

    //: private func makeContentView() -> UIView {
    private func untilBlock() -> UIView {
        //: let view  = UIView()
        let view = UIView()
        //: return view
        return view
    ...
    private func isRow() -> UITableView {
    ...
    private func availablePush() {
        ...
        mTable.register(CustomMenuCell.self, forCellReuseIdentifier: (String(str_shareValue)))
    ...
    func bottomContent() {
        ...
        var dict = [String: Any]()
        //: dict["roomId"] = roomID
        dict[(String(str_sectionContent.prefix(6)))] = roomID
        //: dict["type"] = "3"
        dict[(String(str_viewName))] = "3"
        //: dict["page"] = String(pageIndex)
        dict[(String(str_onTitle))] = String(pageIndex)
        ...
        AstatineThen.share.customAction(params: dict, completion: { array in
            ...
            if self.pageIndex == 0 {
                ...
                let model = GrantUserMeasurable()
                //: model.headPic = "icon_chats_pcr"
                model.headPic = (String(str_myTitle) + String(str_sizeValue.prefix(5)))
                //: model.nickname = "All Numbers".localized
                model.nickname = (String(str_colorData.prefix(4)) + String(str_upValue)).localized
                //: model.uid = "all"
                model.uid = (str_componentName.replacingOccurrences(of: "on", with: "ll"))
...
extension MakeMenuView: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: (String(str_shareValue)), for: indexPath) as! CustomMenuCell
        ...
        if recentmenuArray.count > 0 && indexPath.section == 0 {
            ...
            cellModel = recentmenuArray[indexPath.row] as! GrantUserMeasurable
            //: }else {
        } else {
            //: cellModel = menuArray[indexPath.row] as! TalkingChatRoomMemberModel
            cellModel = menuArray[indexPath.row] as! GrantUserMeasurable
        }
        //: cell.setDropMemberMenuCell(model: cellModel)
        cell.cellWithInfo(model: cellModel)
        //: return cell
        return cell
    }
    ...
    func tableView(_: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        let headeView = UILabel(frame: CGRect(x: 0, y: 0, width: kLet_cameraValue, height: 23))
        //: headeView.backgroundColor = UIColor.appBgColor()
        headeView.backgroundColor = UIColor.imageColor()
        //: let titleLB = UILabel.init()
        let titleLB = UILabel()
        //: titleLB.frame = CGRect(x: 3, y: 5, width: 100, height: 14)
        titleLB.frame = CGRect(x: 3, y: 5, width: 100, height: 14)
        //: titleLB.font = .pingfangFont(type: .Medium, fontSize: 12)
        titleLB.font = .pingfang(type: .Medium, fontSize: 12)
        //: titleLB.textColor = .publishView()
        titleLB.textColor = .publishView()
        //: if recentmenuArray.count > 0 && section == 0 {
        if recentmenuArray.count > 0 && section == 0 {
            //: titleLB.text = "Recent Gifts".localized
            titleLB.text = (String(str_barInputName)).localized
            //: }else {
        } else {
            //: titleLB.text = "All Numbers".localized
            titleLB.text = (String(str_colorData.prefix(4)) + String(str_upValue)).localized
        }
