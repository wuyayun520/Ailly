...
fileprivate let str_toValue:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_coinxxx" :*/
fileprivate let str_nameData:String = "icon_available available list title"
fileprivate let str_tempData:String = "coindatax"

/*: "icon_gift_num_bg" :*/
fileprivate let str_contentTitle:String = "enter let self chooseicon_g"
fileprivate let str_indexName:String = "ift_color direction color"

/*: "x1" :*/
fileprivate let str_placeName:[Character] = ["x","1"]

/*: " x :*/
fileprivate let str_contentValueStartName:String = " xinterest each"
...
class WithContentReusableView: UICollectionViewCell {
    //: var model = TalkingRoomGiftModel()
    var model = VaporBathHandyJSON()
    //: let checked = false
    let checked = false
    //: let giftNum = 0
    let giftNum = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_toValue.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var nameLB: UILabel = {
        ...
        lb.font = UIFont.pingfang(type: .Regular, fontSize: 10)
        //: lb.textColor = .white
        lb.textColor = .white
    ...
    lazy var priceBtn: ProgressTalkingButton = {
        //: let btn = TalkingButton.init()
        let btn = ProgressTalkingButton()
        //: btn.setTitleColor(UIColor.white, for: .normal)
        btn.setTitleColor(UIColor.white, for: .normal)
        //: btn.titleLabel?.font = UIFont.pingfangFont(type: .Regular, fontSize: 10)
        btn.titleLabel?.font = UIFont.pingfang(type: .Regular, fontSize: 10)
        //: btn.setImage(UIImage.eponymUp(name: "icon_coinxxx"), for: .normal)
        btn.setImage(UIImage.eponymUp(name: (String(str_nameData.prefix(5)) + str_tempData.replacingOccurrences(of: "data", with: "xx"))), for: .normal)
    ...
    lazy var numBgView: UIImageView = {
        ...
        let img = UIImageView()
        //: let imag = UIImage.eponymUp(name: "icon_gift_num_bg")
        let imag = UIImage.eponymUp(name: (String(str_contentTitle.suffix(6)) + String(str_indexName.prefix(4)) + "num_bg"))
        //: let sizex = imag.size
        let sizex = imag.size
    ...
    lazy var numLb: UILabel = {
        ...
        lb.font = UIFont.pingfang(type: .Regular, fontSize: 9)
        //: return lb
        return lb
        //: }()
    }()
...
extension WithContentReusableView {
    /// 更改选中状态
    //: func setChecked(checked: Bool) {
    func glycerolizeForCheckedWith(checked: Bool) {
        ...
        if checked {
            ...
            bgView.layer.borderColor = UIColor.plaster().cgColor
    ...
    func giftBy(currenmodel: VaporBathHandyJSON, giftType: DataNumber) {
        //: model = currenmodel
        model = currenmodel
        //: giftIcon.setUrlImage(urlStr: model.imgPreview ?? "")
        giftIcon.direct(urlStr: model.imgPreview ?? "")
        //: priceBtn.setTitle("\(model.price)", for: .normal)
        priceBtn.setTitle("\(model.price)", for: .normal)
        //: selectedImageView.setUrlImage(urlStr: model.cornerMark, placeImg: nil)
        selectedImageView.direct(urlStr: model.cornerMark, placeImg: nil)
        ...
        let attributes = [NSAttributedString.Key.font: UIFont.pingfang(type: .Regular, fontSize: 11),
...
extension WithContentReusableView {
    ...
    func admin() {
        ...
        bgView.backgroundColor = UIColor.sessionList()
