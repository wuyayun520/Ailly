...
// __CLOSE_PRINT__
//
//  DeepShowReactiveCompatible.swift
//  AbroadTalking
//
...
class DeepShowReactiveCompatible: UICollectionViewFlowLayout {

    //: var sectionArr = [GiftDataType]()  
    var sectionArr = [DataNumber]()  // collection有多少个分区
    //: var hotPage = 0                    
    var hotPage = 0                    // hot页数
    ...
    private var GiftItemHeight  = Int(kLet_okText/2) // 每个礼物的高度
    ...
    override func prepare() {
        ...
        layoutContentSize = CGSize(width: Int(kLet_cameraValue-10) * totalPage, height: 2*GiftItemHeight)
    }
    ...
    override func layoutAttributesForItem(at indexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
        ...
        if indexPath.section == DataNumber.Vip.rawValue {
            //: page += hotPage
            page += hotPage
        //: } else if indexPath.section == GiftDataType.Bags.rawValue {
        } else if indexPath.section == DataNumber.Bags.rawValue {
        ...
        let itemWidth = Int(kLet_cameraValue-10) / GiftItemColumn
        //: let x = page * Int(ScreenWidth-10) + pageCol * itemWidth
        let x = page * Int(kLet_cameraValue-10) + pageCol * itemWidth
