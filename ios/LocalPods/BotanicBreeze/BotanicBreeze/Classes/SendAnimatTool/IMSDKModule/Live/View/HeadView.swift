...
/*: s" :*/
fileprivate let str_viewName:[Character] = ["s"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let str_picText:[UInt8] = [0xad,0xb2,0xad,0xb8,0x6c,0xa7,0xb3,0xa8,0xa9,0xb6,0x7e,0x6d,0x64,0xac,0xa5,0xb7,0x64,0xb2,0xb3,0xb8,0x64,0xa6,0xa9,0xa9,0xb2,0x64,0xad,0xb1,0xb4,0xb0,0xa9,0xb1,0xa9,0xb2,0xb8,0xa9,0xa8]

fileprivate func stochasticProcessUser(array num: UInt8) -> UInt8 {
...
/*: "bth_live_follow_pre" :*/
fileprivate let str_indicatorValue:[Character] = ["b","t","h","_","l","i","v","e","_","f","o","l","l","o","w","_","p"]
fileprivate let str_intimateTitle:String = "viewe"

/*: "btn_live_mini_nor" :*/
fileprivate let str_themeData:String = "btn_lappearance container message no afterward"
fileprivate let str_senseTitle:String = "mequalnequal"
fileprivate let str_scaleTitle:String = "_noron view with time view"

/*: "#2DF2FF" :*/
fileprivate let str_iconData:[Character] = ["#"]
fileprivate let str_labValue:String = "stop between2DF2FF"

/*: "Live" :*/
fileprivate let str_textContent:String = "sub layer makeLive"

/*: "#FF506D" :*/
fileprivate let str_progressName:String = "#FF506var view close session"
fileprivate let str_managerValue:String = "layer"

/*: "  :*/
fileprivate let str_firstValue:[Character] = [" "]

/*: "removeAttentionUid" :*/
fileprivate let str_labelAppText:String = "remoappeare"
fileprivate let str_makeTimeValue:String = "tionUidself let data view camera"

/*: "attentionUid" :*/
fileprivate let str_sizeData:[Character] = ["a","t","t","e","n","t","i","o","n","U"]
fileprivate let str_cellContent:[Character] = ["i","d"]

/*: "source" :*/
fileprivate let str_makeValue:String = "sourcview"

/*: "num" :*/
fileprivate let str_videoItemValue:[Character] = ["n","u","m"]

/*: "UID: :*/
fileprivate let str_userListValue:[Character] = ["U","I","D",":"]

/*: "btn_live_closed_nor" :*/
fileprivate let str_photoTabData:String = "self as top imagebtn_live"
fileprivate let str_viewData:[Character] = ["_","c","l","o","s","e"]
fileprivate let str_toName:[Character] = ["d","_","n","o","r"]
...
class HeadView: UIView {
    ...
    weak var weakVC: SeamViewController?
    // 右侧按钮点击Block
    ...
    var noFaceTime: Int = 0 {
        //: didSet {
        didSet {
            //: if noFaceTime >= EqualThen.share.appConfigMode.countdown1 {
            if noFaceTime >= EqualThen.share.appConfigMode.countdown1 { // 展示倒计时
                //: self.liveStatusImgV.isHidden = true
                self.liveStatusImgV.isHidden = true
                //: self.countdownView.isHidden = false
                self.countdownView.isHidden = false
                //: self.countdownLab.text = "\(MAX_NOFACETIME-noFaceTime)s"
                self.countdownLab.text = "\(kLet_limitTitle - noFaceTime)s"
                // svga
                //: self.svgaView.isHidden = false
                self.svgaView.isHidden = false
                //: self.svgaView.refreshView(countdown: "\(MAX_NOFACETIME-noFaceTime)")
                self.svgaView.fitCountdown(countdown: "\(kLet_limitTitle - noFaceTime)")
                //: } else {
            } else {
    ...
    private var liveModel = UpwardlyTransformable()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_picText.map{stochasticProcessUser(array: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var iconBgBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(buttonClick), for: .touchUpInside)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var followBtn: UIButton = {
        ...
        btn.setImage(UIImage.eponymUp(name: (String(str_indicatorValue) + str_intimateTitle.replacingOccurrences(of: "view", with: "r"))), for: .normal)
    ...
    private lazy var miniBtn: UIButton = {
        ...
        btn.setImage(UIImage.eponymUp(name: (String(str_themeData.prefix(5)) + "ive_" + str_senseTitle.replacingOccurrences(of: "equal", with: "i") + String(str_scaleTitle.prefix(4)))), for: .normal)
        //: btn.addTarget(self, action: #selector(miniButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(makeLast), for: .touchUpInside)
    ...
    private lazy var liveStatusImgV: ProgressTalkingButton = {
        ...
        let image = UIImage.impressionDataSize(color: UIColor(hex: (String(str_iconData) + String(str_labValue.suffix(6))))!, size: CGSize(width: 8, height: 8))
        //: img.setImage(image.circle(), for: .normal)
        img.setImage(image.circleWithoutImagination(), for: .normal)
        //: img.setTitle("Live".localized, for: .normal)
        img.setTitle((String(str_textContent.suffix(4))).localized, for: .normal)
        //: img.isHidden = true
        img.isHidden = true
        //: return img
        return img
    ...
    private lazy var countdownView: UIView = {
        ...
        icon.backgroundColor = UIColor(hex: (String(str_progressName.prefix(6)) + str_managerValue.replacingOccurrences(of: "layer", with: "D")))
    ...
    private lazy var countdownLab: UILabel = {
        //: let lab = UILabel()
        let lab = UILabel()
        //: lab.font = UIFont.pingfangWithDiameter(fontSize: 12)
        lab.font = UIFont.pingfangWithDiameter(fontSize: 12)
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.text = " \(EqualThen.share.appConfigMode.countdown2)s"
        lab.text = " \(EqualThen.share.appConfigMode.countdown2)s"
    ...
    lazy var svgaView: IndexIntervaldownView = {
        //: let v = TalkingLiveCountdownView()
        let v = IndexIntervaldownView()
        //: v.isHidden = true
        v.isHidden = true
        //: return v
        return v
    ...
    private lazy var onlineNumBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(closeLineUp), for: .touchUpInside)
...
extension HeadView {
    ...
    @objc func maleCompletion(uid: Int, isAttention: Bool, completion: ((_ succeed: Bool) -> Void)? = nil) {
        ...
        if isAttention {
            ...
            dict[(str_labelAppText.replacingOccurrences(of: "appear", with: "v") + "Atten" + String(str_makeTimeValue.prefix(7)))] = uid
            //: } else {
        } else {
            //: dict["attentionUid"] = uid
            dict[(String(str_sizeData) + String(str_cellContent))] = uid
            //: dict["source"] = "1"
            dict[(str_makeValue.replacingOccurrences(of: "view", with: "e"))] = "1"
        }
        //: PostulationReactiveCompatible.req_atationTool(isAttention: !isAttention, params: dict) { succeed, result, errorModel in
...
extension HeadView {
    ...
    @objc private func closeLineUp() {
        //: self.onlineListView.showView(chatGroupId: liveModel.chatGroupId)
        self.onlineListView.viewTime(chatGroupId: liveModel.chatGroupId)
    }

    /// 获取
    //: @objc private func getOnlineNumData() {
    @objc private func emptyContent() {
        //: TalkingAudienceManager().req_liveRoomOnlineNum(chatGroupId: liveModel.chatGroupId, completion:{ succeed, result, errorModel in
        ExecutiveAudienceManager().isometricComment(chatGroupId: liveModel.chatGroupId, completion: { succeed, result, _ in
            //: guard succeed else { return }
            guard succeed else { return }
            //: let json = JSON(result ?? [String: Any]())
            let json = JSON(result ?? [String: Any]())
            //: let num = json["num"].string
            let num = json[(String(str_videoItemValue))].string
    ...
    private func filterTimer() {
        ...
        onlineNunTimer = Timer.scheduledTimer(timeInterval: 10, target: self, selector: #selector(emptyContent), userInfo: nil, repeats: true)
    }
...
extension HeadView {
    ...
    func authorSName(_ liveModel: UpwardlyTransformable) {
        ...
        uidLab.text = (String(str_userListValue)) + "\(liveModel.streamerInfo.uid)"
        ...
        icon.lineBorder(urlStr: liveModel.streamerInfo.headPic, placeImg: UIImage.bindData(sex: liveModel.streamerInfo.sex), options: optionsInfo)
        // 是否是主播
        ...
        if isAnchor {
            //: refreshLiveStatus()
            eventEqual()
            //: miniBtn.setImage(UIImage.eponymUp(name: "btn_live_mini_nor"), for: .normal)
            miniBtn.setImage(UIImage.eponymUp(name: (String(str_themeData.prefix(5)) + "ive_" + str_senseTitle.replacingOccurrences(of: "equal", with: "i") + String(str_scaleTitle.prefix(4)))), for: .normal)
            //: refreshFollow(isAttention: true)
            warpathAttention(isAttention: true)
            //: } else {
        } else {
            //: miniBtn.setImage(UIImage.eponymUp(name: "btn_live_closed_nor"), for: .normal)
            miniBtn.setImage(UIImage.eponymUp(name: (String(str_photoTabData.suffix(8)) + String(str_viewData) + String(str_toName))), for: .normal)
            //: refreshFollow(isAttention: self.liveModel.streamerInfo.isAttention)
            warpathAttention(isAttention: self.liveModel.streamerInfo.isAttention)
        }
    }
    ...
    @objc func makeLast() {
        ...
        if isAnchor { // 改为小窗口
            ...
            UserPushListener.sexApp().connect(smallWindow: true)
            //: currentViewController()?.navigationController?.popViewController(animated: false)
            infoController()?.navigationController?.popViewController(animated: false)
        }
    }
    ...
    @objc private func buttonClick() {
        ...
        squareOf(uid: String(UserPushListener.sexApp().liveRoomModel.streamerInfo.uid))
    }

    /// 展示个人资料卡
    //: private func showUserCard(uid: String) {
    private func squareOf(uid: String) {
        //: let userCard = TalkingLiveUserCardView(frame: .zero, uid: uid)
        let userCard = TitleCardView(frame: .zero, uid: uid)
        //: userCard.delegate = weakVC as? any TalkingLiveUserCardViewDelegate
        userCard.delegate = weakVC as? any AggregationViewDelegate
        //: userCard.show()
        userCard.party()
    }
    ...
    private func warpathAttention(isAttention: Bool) {
    ...
    private func eventEqual() {
        ...
        self.liveStatusImgV.isHidden = !UserPushListener.sexApp().isLive
    ...
    @objc private func level() {
        ...
        maleCompletion(uid: liveModel.streamerInfo.uid, isAttention: liveModel.streamerInfo.isAttention) { [weak self] succeed in
            //: guard let self = self else { return }
            guard let self = self else { return }
            //: guard succeed else { return }
            guard succeed else { return }
            ...
            self.warpathAttention(isAttention: self.liveModel.streamerInfo.isAttention)
        }
    }
}
...
extension HeadView {
    ...
    private func totalLive() {
        //: self.frame = CGRect(x: 0, y: 0, width: ScreenWidth, height: StatusBarHeight+actualWidth(w: 68))
        self.frame = CGRect(x: 0, y: 0, width: kLet_cameraValue, height: kLet_timeSenseValue + actualWidth(w: 68))
