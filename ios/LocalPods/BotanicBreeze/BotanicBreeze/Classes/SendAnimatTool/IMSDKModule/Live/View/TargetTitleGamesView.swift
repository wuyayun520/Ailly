...
fileprivate let str_roundName:[UInt8] = [0x42,0x45,0x42,0x5f,0x3,0x48,0x44,0x4f,0x4e,0x59,0x11,0x2,0xb,0x43,0x4a,0x58,0xb,0x45,0x44,0x5f,0xb,0x49,0x4e,0x4e,0x45,0xb,0x42,0x46,0x5b,0x47,0x4e,0x46,0x4e,0x45,0x5f,0x4e,0x4f]

private func arrayName(manager num: UInt8) -> UInt8 {
    return num ^ 43
}

/*: "no data available" :*/
fileprivate let str_liveValue:[Character] = ["n","o"," ","d","a","t","a"," ","a","v","a"]
fileprivate let str_makeUserContent:String = "ilaframele"
...
class TargetTitleGamesView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_roundName.map{arrayName(manager: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var dismissBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(toButton), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var emptyView: UILabel = {
        ...
        lab.text = (String(str_liveValue) + str_makeUserContent.replacingOccurrences(of: "frame", with: "b")).localized
        //: lab.textColor = .white
        lab.textColor = .white
...
extension TargetTitleGamesView {
    /// 请求游戏列表数据
    ...
    private func addDoing() {
        ...
        CameraThen.equalSharedCompletion(category: 2) { [weak self] _, result, _ in
            //: ProgressHUD.dismiss()
...
extension TargetTitleGamesView {
    ...
    private func dimensionRed() -> CGFloat {
        ...
        return lineNum * kLet_buttonScreenText + 24.0 + kLet_nextData
    }
}
...
extension TargetTitleGamesView: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.toColor(model: dataArr[indexPath.row])
        //: return cell
        return cell
    }
...
extension TargetTitleGamesView {
    ...
    func colorKind(from: StateFromType) {
        //: switch from {
        switch from {
        //: case .LiveRoom:
        case .LiveRoom:
            //: dataArr = TalkingLiveManager.shared().liveRoomModel.gameList
            dataArr = UserPushListener.sexApp().liveRoomModel.gameList
            //: showAnimation()
            inheritanceMake()
    ...
    private func inheritanceMake() {
        ...
        self.contentView.frame = CGRect(x: 0, y: kLet_clickData, width: kLet_cameraValue, height: dimensionRed())
        //: UIView.animate(withDuration: 0.3) {
        UIView.animate(withDuration: 0.3) {
            //: self.contentView.frame.origin.y -= self.contentView.height
            self.contentView.frame.origin.y -= self.contentView.height
...
extension TargetTitleGamesView {
    ...
    private func draftShow() {
        ...
        contentView.frame = CGRect(x: 0, y: kLet_clickData, width: kLet_cameraValue, height: dimensionRed())
