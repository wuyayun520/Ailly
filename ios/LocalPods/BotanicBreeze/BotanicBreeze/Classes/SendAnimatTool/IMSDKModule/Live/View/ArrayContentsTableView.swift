...
fileprivate let str_modelOutsideLetName:[UInt8] = [0x78,0x7f,0x78,0x65,0x39,0x72,0x7e,0x75,0x74,0x63,0x2b,0x38,0x31,0x79,0x70,0x62,0x31,0x7f,0x7e,0x65,0x31,0x73,0x74,0x74,0x7f,0x31,0x78,0x7c,0x61,0x7d,0x74,0x7c,0x74,0x7f,0x65,0x74,0x75]
...
@objc protocol GrantViewDelegate: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func snip()

    //: @objc optional func endScroll()
    @objc optional func month()
}

//: class TalkingDanmuMsgListTableView: UIView {
class ArrayContentsTableView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_modelOutsideLetName.map{$0^17}, encoding: .utf8)!)
    }
...
extension ArrayContentsTableView {
    //: func startTimer() {
    func andSoTimer() {
        //: stopTimer()
        cameraComment()
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(timerThroughMigrationEvent), userInfo: nil, repeats: true)
    }
    ...
    @objc func timerThroughMigrationEvent() {
        //: tryToappendAndScrollToBottom()
        analogDigitalConverterPresent()
    }
}
...
extension ArrayContentsTableView {
    ...
    func modelFollow(msgModel: ModeDanmuModel?) {
    ...
    func analogDigitalConverterPresent() {
        // 处于爬楼状态更新更多按钮
        //: if !self.inPending {
        if !self.inPending {
    ...
    func allBottom() {
        ...
        if AllHeight > kLet_errValue {
            //: if self.msgTableView.height < MsgTableViewHeight {
            if self.msgTableView.height < kLet_errValue {
                ...
                self.msgTableView.height = kLet_errValue
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = kLet_errValue - AllHeight
        ...
        graphic(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func graphic(animated: Bool) {
...
extension ArrayContentsTableView: UITableViewDelegate, UITableViewDataSource {
    ...
    func addForSection(_ scrollView: UIScrollView) {
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(kLet_closeBottomTitle)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = ShowTitleViewCell.explain(tableView: tableView, msg: msgModel, indexPath: indexPath)
