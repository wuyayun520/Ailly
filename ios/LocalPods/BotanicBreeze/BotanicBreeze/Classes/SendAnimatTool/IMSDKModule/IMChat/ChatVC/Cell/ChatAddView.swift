...
/*: "icon_chat_miss" :*/
fileprivate let str_listValue:String = "aton"
fileprivate let str_hiddenName:String = "t_missadd in else up leading"

/*: "icon_chat_yes" :*/
fileprivate let str_addName:[Character] = ["i","c","o","n","_"]
fileprivate let str_toValue:String = "self modelchat_yes"
...
class ChatAddView: IndexViewCell {
//    var transBlock: (() -> Void)?

    //: var textData: SendCellData?
    var textData: SendCellData?
...
extension ChatAddView {
    //: override func fill(with data: TCommonCellData) {
    override func fill(with data: TCommonCellData) {
        //: super.fill(with: data)
        super.fill(with: data)

        //: if let temTextData = data as? SendCellData {
        if let temTextData = data as? SendCellData {
...
//                self.messageData?.translateStatus != .MsgTranslate_succeed && self.messageData?.msgModel.msgInfo.callCmd.count ?? 0 <= 0 && temTextData.msgModel.msgInfo.messageType != .MessageTypeWelcome && "\(temTextData.msgModel.user.uid)" != UserMacroDefine.getXiaoMiID()) { // 对方发的 && 开启了翻译功能 && 消息未翻译成功
...
//            }
            ...
            if temTextData.msgModel.quoteMsgInfo.coverMsg() {
            ...
            if self.textData?.translateStatus == .MsgTranslate_translating {
                ...
                self.transStatusImagV.image = UIImage.eponymUp(name: (str_listValue.replacingOccurrences(of: "at", with: "ic") + "_cha" + String(str_hiddenName.prefix(6))))
                ...
                self.transStatusImagV.image = UIImage.eponymUp(name: (String(str_addName) + String(str_toValue.suffix(8))))
            }
    ...
    func area() {
        ...
        let containerTap = UITapGestureRecognizer(target: self, action: #selector(touchKeyTap(tap:)))
        ...
        let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == UserMacroDefine.underGet())
        //: if isOfficial == false {
        if isOfficial == false {
            //: addLongTagGes()
            viewGes()
