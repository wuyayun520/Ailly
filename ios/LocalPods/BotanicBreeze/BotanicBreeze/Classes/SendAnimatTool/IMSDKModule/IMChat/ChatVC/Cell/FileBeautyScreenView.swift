...
fileprivate let str_kitData:[UInt8] = [0x19,0x1e,0x19,0x4,0x58,0x13,0x1f,0x14,0x15,0x2,0x4a,0x59,0x50,0x18,0x11,0x3,0x50,0x1e,0x1f,0x4,0x50,0x12,0x15,0x15,0x1e,0x50,0x19,0x1d,0x0,0x1c,0x15,0x1d,0x15,0x1e,0x4,0x15,0x14]

private func nameLabelTask(info num: UInt8) -> UInt8 {
    return num ^ 112
}

/*: "move" :*/
fileprivate let str_pointData:String = "momake"
...
class FileBeautyScreenView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_kitData.map{nameLabelTask(info: $0)}, encoding: .utf8)!)
    }
    ...
    func database(_ danumuMsgModel: ViewScreenModel?) {
    ...
    @objc func equalAction(_: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(equalAction(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension FileBeautyScreenView: UIGestureRecognizerDelegate {
    ...
    func sex(for danumuMsgModel: ViewScreenModel?, isFirst: Bool) {
        //: let itemView = TalkingGameFloatingScreenItemView(model: danumuMsgModel)
        let itemView = StopItemView(model: danumuMsgModel)
        ...
        var frame = CGRect(x: kLet_cameraValue, y: isFirst ? 7 : 50, width: itemView.widthValue, height: 24)
        //: var endFrame = frame
        var endFrame = frame
        ...
        if FileReactiveCompatible.shared.direction == .rightToLeft {
            ...
            endFrame.origin.x = kLet_cameraValue
        }
        //: addSubview(itemView)
        addSubview(itemView)
        ...
        if let anim = POPBasicAnimation(propertyNamed: kPOPViewFrame) {
            ...
            itemView.pop_add(anim, forKey: (str_pointData.replacingOccurrences(of: "make", with: "ve")))
...
extension FileBeautyScreenView {
    // 添加视图
    //: private func setupSubviews() {
    private func voiceRemove() {
