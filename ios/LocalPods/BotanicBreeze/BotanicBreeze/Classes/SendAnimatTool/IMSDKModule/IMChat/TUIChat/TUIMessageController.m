...
- (void)setupViews {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(menuWillHide:)
                                               ...
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onNewMessage:) name:[EditNotificationTool goEnable] object:nil];
...
- (void)loadMessage {
    ...
    if (self.currentChatType == TalkingIMChatType_group ||
        ...
        [[V2TIMManager sharedInstance] getGroupHistoryMessageList:self.targetID count:msgCount lastMsg:self.msgForGet succ:^(NSArray<V2TIMMessage *> *msgs) {
            ...
            if ([TalkingConversationListener shared].isConnection) { //推送点进来会报错
                NSString *str = [@"Net Error, Try again later" localized];
                [self oversightWithShowMsg:[str localized]];
            }
        ...
        [[V2TIMManager sharedInstance] getC2CHistoryMessageList:self.targetID count:msgCount lastMsg:self.msgForGet succ:^(NSArray<V2TIMMessage *> *msgs) {
            ...
            if ([TalkingConversationListener shared].isConnection) {                
                if (EqualThen.share.appStatus == 0) {
                    NSString *str = [@"Net Error, Try again later" localized];
                    [self oversightWithShowMsg:[str localized]];
                }
            }
...
- (void)getMessages:(NSArray *)msgs msgCount:(int)msgCount {
    ...
    for (NSInteger k = array.count - 1; k >= 0; --k) {
        ...
        if ([userId isEqualToString:EqualThen.share.loginUserMode.userID]) {
...
- (void)onNewMessage:(NSNotification *)notification
{
    ...
    if ([msg.sender isEqualToString:EqualThen.share.loginUserMode.userID]) {
        //过滤本人发送的消息
        return;
    }
    ...
    if (uiMsgs.count > 0) {
        ...
        if ((self.tableView.contentSize.height - self.tableView.contentOffset.y) < [UserMacroDefine heightCard] * 1.5) {
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //    缓存存在问题待处理
    //    if (height > 0 && ![data isKindOfClass:[AbShowCellData class]] && ![data isKindOfClass:[EponymCellData class]]) {
    //        return height;
    //    }
...
- (void)sendCustomMessage:(TUIMessageCellData *)msg {
    ...
    NSInteger sender = EqualThen.share.loginUserMode.userID.integerValue;
    NSString *msgID;
    if (isGroup) {
...
- (void)modifyCustomMessageDataWithExtral:(NSDictionary *)extral cellData:(TUIMessageCellData *)msg {
    ...
    // 更新内存模型
    msg.msgModel = [DiskModel modelWithDic:newExtraDict];
}
...
///   - msg: 图片消息
///   - model: 私密照片模型
- (void)changeIntimatePhotoMsg:(TUIMessageCellData *)msg photoModel:(ImageArrayModel *)model {
    dispatch_async(dispatch_get_main_queue(), ^{
        if (model == nil) { return; }
        [ImageArrayModel renderForStatus:model custom:msg.msgID];
...
///   - msg: 视频消息
///   - model: 私密视频模型
- (void)changeIntimateVideoMsg:(TUIMessageCellData *)msg videoModel:(PlayCollectionModel *)model {
    dispatch_async(dispatch_get_main_queue(), ^{
        if (model == nil) { return; }
        [PlayCollectionModel gallery:model sizeId:msg.msgID];
...
- (void)changeIntimateGiftMsg:(TUIMessageCellData *)msg {
    dispatch_async(dispatch_get_main_queue(), ^{
        [StreetSmartJsonModel method:msg.msgID];
...
- (TalkingSystemMsgCellData *)transSystemMsgFromDate:(NSDate *)date
{
    if(_msgForDate == nil || fabs([date timeIntervalSinceDate:_msgForDate.timestamp]) > MAX_MESSAGE_SEP_DLAY) {
        ...
        system.contentStr = [NSDate someFirstWithShowDetail:YES date:date];
...
- (void)onLongPressMessageWithCell:(TUIMessageCell *)cell{
    ...
    BOOL hasJumps = NO;
    if ([data isKindOfClass:[SendCellData class]]) {
        SendCellData *textCellData = (SendCellData *)data;
    ...
    // 只有文字才有复制功能
    if (([data isKindOfClass:[EnableCellData class]] ||
         [data isKindOfClass:[SendCellData class]]) && hasJumps == NO) {
        // 过滤掉音视频通话文本消息
...
- (NSArray *)requiredResponseWithLongGestureRecognizer {
    return @[[EnableCellData class],
             [SendCellData class],
             [AtHeadCellData class],
             [DataReactiveCompatible class],
             [ManageCellData class],
             [SoulCellData class],
             [PlayReactiveCompatible class],
             [CompartmentSourceCellData class]];
}
...
- (void)onCopyMsg:(id)sender {
    ...
    if ([_menuUIMsg isKindOfClass:[EnableCellData class]] ||
        [_menuUIMsg isKindOfClass:[SendCellData class]]) {
        SendCellData *txtMsg = (SendCellData *)_menuUIMsg;
...
- (BOOL)canReplyResponseWithLongGestureRecognizer:(TUIMessageCellData *)cellData {
    ...
    if ([cellData isKindOfClass:[SoulCellData class]] || [cellData isKindOfClass:[AtHeadCellData class]] || [cellData isKindOfClass:[CompartmentSourceCellData class]]) {
        return YES;
    }
...
- (void)translateMsgWithCellData:(TUIMessageCellData *)cellData {
    ...
    if (EqualThen.share.appUserConfigMode.enableTranslate == NO) { // 非会员，无法使用翻译
        [[EditTextThen share] whole];
        return;
    }
    
    if ([cellData isKindOfClass:[SendCellData class]]) {
        SendCellData *txtMsg = (SendCellData *)cellData;
        [self translateMessageToReq:txtMsg andType:0];
        
    } else if ([cellData isKindOfClass:[EnableCellData class]]) {
        EnableCellData *txtMsg = (EnableCellData *)cellData;
...
- (void)translateMessageToReq:(TUIMessageCellData *)txtMsg andType:(int)type {
    ...
    // 调用翻译接口
    [PostulationReactiveCompatible dismissCompletionWithMsgId:txtMsg.msgModel.msgInfo.msgId type:type completion:^(BOOL succeed, NSDictionary *dict, NSInteger errorCode) {
        if (!succeed) {
            if (errorCode == 403) { // 非会员，无法使用翻译
                EqualThen.share.appUserConfigMode.enableTranslate = NO;
                [[EditTextThen share] whole];
            }
