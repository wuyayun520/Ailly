...
fileprivate let str_noName:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "all" :*/
fileprivate let str_mText:[Character] = ["a","l","l"]
...
class AddMsgCell: IndexViewCell {
    //: var giftData: HeadCellData?
    var giftData: HeadCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_noName.reversed(), encoding: .utf8)!)
    }
...
extension AddMsgCell {
    ...
    override func fill(with data: TCommonCellData) {
        ...
        super.fill(with: data)
        //: self.giftData = data as? HeadCellData
        self.giftData = data as? HeadCellData
        ...
        self.giftImgView.direct(urlStr: imgPreview)
        //: self.giftContentLab.attributedText = giftData.attributedString
        self.giftContentLab.attributedText = giftData.attributedString
        //: if giftData.msgModel.toUser.uid == "all" { // 送礼所有人
        if giftData.msgModel.toUser.uid == (String(str_mText)) { // 送礼所有人
            //: self.toUserImgView.image = UIImage.eponymUp(name: giftData.msgModel.toUser.headPic)
            self.toUserImgView.image = UIImage.eponymUp(name: giftData.msgModel.toUser.headPic)
            //: } else {
        } else {
            //: self.toUserImgView.setUrlImage(urlStr: giftData.msgModel.toUser.headPic)
            self.toUserImgView.direct(urlStr: giftData.msgModel.toUser.headPic)
        }
    }
