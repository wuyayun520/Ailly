...
/*: "#ffffff" :*/
fileprivate let str_requestTitle:String = "#"
fileprivate let str_addData:String = "numbernumbernumber"

/*: "earned" :*/
fileprivate let str_photoData:[Character] = ["e","a","r","n","e","d"]

/*: "Coins in" :*/
fileprivate let str_dataContent:String = "Coins inhidden view cell view room"

/*: "Error:  :*/
fileprivate let str_toTitle:[Character] = ["E","r","r","o","r",":"," "]

/*: "#C946D2" :*/
fileprivate let str_modelName:String = "self other self#C946D2"

/*: "btn_chat_floatS_userBK_nor" :*/
fileprivate let str_indexName:String = "btn_return block from key data"
fileprivate let str_bottomMakeData:String = "_floatmake make error"
fileprivate let str_pathMillData:String = "rBK_norvalue guard video equal"
...
class StopItemView: UIView {
    ...
    private let TextColor = (str_requestTitle.capitalized + str_addData.replacingOccurrences(of: "number", with: "ff"))

    //: var clickBlock: (() -> Void)?
    var clickBlock: (() -> Void)?

    //: convenience init(model: TalkingGameFloatingScreenModel?) {
    convenience init(model: ViewScreenModel?) {
        //: self.init()
        self.init()
    ...
    private func colorEqual() {
        ...
        if danumuMsgModel.bgPic.count > 0 {
            ...
            occurrence(withUrl: danumuMsgModel.bgPic)
        }
        ...
        if danumuMsgModel.content.count > 0 {
            ...
            do {
                ...
                if let range = attrStr.string.range(of: (String(str_photoData))) {
                ...
                if let range = attrStr.string.range(of: (String(str_dataContent.prefix(8)))) {
                    //: attrStr.addAttributes([.foregroundColor: UIColor.white], range: NSRange(range, in: attrStr.string))
                    attrStr.addAttributes([.foregroundColor: UIColor.white], range: NSRange(range, in: attrStr.string))
                }
                //: attrStr.addAttributes([.font: UIFont.pingfangFont(type: .Medium, fontSize: 12)], range: NSRange(location: 0, length: attrStr.length))
                attrStr.addAttributes([.font: UIFont.pingfang(type: .Medium, fontSize: 12)], range: NSRange(location: 0, length: attrStr.length))
                ...
                printLog(message: (String(str_toTitle)) + "\(error)")
            }
        }
    }

    //: func updateBackground(withUrl url: String) {
    func occurrence(withUrl url: String) {
        ...
        self.backgroundImageView.lineBorder(urlStr: url) { [weak self] _, image in
            ...
            if let image {
                ...
                if image.size.height > self.HeightOfBackgroundImageView {
                    ...
                    newImage = image.sweat(with: CGSize(width: targetWidth, height: self.HeightOfBackgroundImageView))
                }
            }
    ...
    var danumuMsgModel: ViewScreenModel? {
        //: didSet {
        didSet {
            //: configModel()
            colorEqual()
    ...
    var totalTime: CGFloat {
        ...
        return (kLet_cameraValue + widthValue) * SpendTime / kLet_cameraValue
    }

    //: var totalTimeOfLine: CGFloat {
    var totalTimeOfLine: CGFloat {
        //: return widthValue * LineSpendTime / ScreenWidth
        return widthValue * LineSpendTime / kLet_cameraValue
    }

    //: var hide2ShowTime: CGFloat {
    var hide2ShowTime: CGFloat {
        //: return widthValue * SpendTime / ScreenWidth
        return widthValue * SpendTime / kLet_cameraValue
    }
    ...
    lazy var headAnimatedImageView: UIImageView = {
        ...
        h.layer.borderColor = UIColor(hex: (String(str_modelName.suffix(7))))?.cgColor
        //: return h
        return h
        //: }()
    }()
    ...
    lazy var headBackImageView: UIImageView = {
        ...
        h.contentMode = .scaleAspectFill
        //: h.image = UIImage.eponymUp(name: "btn_chat_floatS_userBK_nor")
        h.image = UIImage.eponymUp(name: (String(str_indexName.prefix(4)) + "chat" + String(str_bottomMakeData.prefix(6)) + "S_use" + String(str_pathMillData.prefix(7))))
        //: return h
        return h
        //: }()
    }()
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(reverence(_:)))
        //: return t
        return t
        //: }()
    }()
...
extension StopItemView {
    /// 点击跳转事件
    //: @objc func tapAction(_ tapGesture: UITapGestureRecognizer) {
    @objc func reverence(_: UITapGestureRecognizer) {
        //: guard let gameUrl = self.danumuMsgModel?.gameUrl else { return }
        guard let gameUrl = self.danumuMsgModel?.gameUrl else { return }
        //: let httpsUrl = EqualThen.share.appConfigMode.urlH5Domain+gameUrl
        let httpsUrl = EqualThen.share.appConfigMode.urlH5Domain + gameUrl
        ...
        config.clearBgColor = true
        //: EditTextThen.share.func__pushToWebVC(urlStr: httpsUrl, webConfig: config)
        EditTextThen.share.bileOf(urlStr: httpsUrl, webConfig: config)
    }
}
