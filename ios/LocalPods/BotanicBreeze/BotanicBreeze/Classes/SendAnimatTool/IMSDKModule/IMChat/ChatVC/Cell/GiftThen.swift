...
fileprivate let str_endData:[UInt8] = [0x4e,0x53,0x4e,0x59,0xd,0x48,0x54,0x49,0x4a,0x57,0x1f,0xe,0x5,0x4d,0x46,0x58,0x5,0x53,0x54,0x59,0x5,0x47,0x4a,0x4a,0x53,0x5,0x4e,0x52,0x55,0x51,0x4a,0x52,0x4a,0x53,0x59,0x4a,0x49]

fileprivate func toCancelData(domain num: UInt8) -> UInt8 {
...
fileprivate let str_maxText:[Character] = ["i","c","o","n","_","c"]
fileprivate let str_statusModelTitle:[Character] = ["h","a","t","_"]
fileprivate let str_mData:String = "trcookien"

/*: "icon_chat_translate_pre" :*/
fileprivate let str_partName:String = "need app receive pushicon_c"
fileprivate let str_makeName:String = "viewte"

/*: "icon_chat_miss" :*/
fileprivate let str_tableTitle:[Character] = ["i","c","o","n","_"]
fileprivate let str_labelText:String = "array action title framechat_miss"

/*: "icon_chat_yes" :*/
fileprivate let str_findName:[Character] = ["i","c","o","n","_","c","h","a","t","_","y","e","s"]
...
class GiftThen: IndexViewCell {
    //: var transBlock: (() -> Void)?
    var transBlock: (() -> Void)?

    //: var textData: EnableCellData?
    var textData: EnableCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_endData.map{toCancelData(domain: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var transLB: YYLabel = {
        ...
        //: label.numberOfLines = 0
        label.numberOfLines = 0
        //: label.font = UIFont.outsideAllSize(fontSize: 16)
        label.font = UIFont.outsideAllSize(fontSize: 16)
        //: return label
        return label
        //: }()
    ...
    lazy var transBtn: UIButton = {
        ...
        let btn = UIButton()
        //: btn.setImage(UIImage.eponymUp(name: "icon_chat_translate_nor"), for: .normal)
        btn.setImage(UIImage.eponymUp(name: (String(str_maxText) + String(str_statusModelTitle) + str_mData.replacingOccurrences(of: "cookie", with: "a") + "slate_nor")), for: .normal)
        //: btn.setImage(UIImage.eponymUp(name: "icon_chat_translate_pre"), for: .highlighted)
        btn.setImage(UIImage.eponymUp(name: (String(str_partName.suffix(6)) + "hat_trans" + str_makeName.replacingOccurrences(of: "view", with: "la") + "_pre")), for: .highlighted)
        //: btn.addTarget(self, action: #selector(transBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(popSearchClick), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension GiftThen {
    ...
    override func fill(with data: TCommonCellData) {
        ...
        super.fill(with: data)
        //: textData = data as? EnableCellData
        textData = data as? EnableCellData
        ...
        if imgPreview.count > 0, textData.msgModel.msgInfo.messageType == .MessageTypeWelcome {
            ...
            giftImagV.direct(urlStr: imgPreview)
        }
        ...
        let maxWidth = UserMacroDefine.loadColor() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        // 翻译赋值相关
        ...
        if textData.translateStatus == .MsgTranslate_translating {
            ...
            self.transShowView.stopAnimating()
            //: self.transStatusImagV.image = UIImage.eponymUp(name: "icon_chat_miss")
            self.transStatusImagV.image = UIImage.eponymUp(name: (String(str_tableTitle) + String(str_labelText.suffix(9))))
            //: } else if textData.translateStatus == .MsgTranslate_succeed {
        } else if textData.translateStatus == .MsgTranslate_succeed {
            //: self.transShowView.stopAnimating()
            self.transShowView.stopAnimating()
            //: self.transStatusImagV.image = UIImage.eponymUp(name: "icon_chat_yes")
            self.transStatusImagV.image = UIImage.eponymUp(name: (String(str_findName)))
        }
