...
fileprivate let str_shareText:[Character] = ["D","o","c","u","m","e","n","t","s","/","U","s","e","r","/","v"]
fileprivate let str_kindViewTitle:String = "intimate whole profile result inputoice/"

/*: "Documents/User/record/" :*/
fileprivate let str_touchSourceName:String = "Doto data return tool pop"
fileprivate let str_progressTitle:String = "cumecustomt"
fileprivate let str_randomContent:String = "main equal let sea end/record/"

/*: "cancelled" :*/
fileprivate let str_labValue:[Character] = ["c","a","n","c","e","l"]
fileprivate let str_toBagData:String = "LED"

/*: .wav" :*/
fileprivate let str_leadingValueTitle:[Character] = [".","w","a","v"]
...
typealias OneKeyGreetDownloadBlock = (_ model: RawTaskModel) -> Void

//: protocol VoiceDownloadTaskManagerDelegate: NSObjectProtocol {
protocol CellManagerDelegate: NSObjectProtocol {
    /// 下载完成
    ...
    func phonation(model: RawTaskModel)
    /// 过期语音消息，服务器缓存时间为7天
...
extension HeadWithDataDelegate {
    /// 添加下载任务
    //: func addDownloadTasks(_ tasksArray: [VoiceDownloadTaskModel]) {
    func createApp(_ tasksArray: [RawTaskModel]) {
        ...
        for model in filterArray {
            ...
            if model.zipFilePath.isEmpty {
                ...
                model.zipFilePath = model.themePlace()
            }

            // 重编译，相对路径会变
            //: if model.zipFilePath.contains("Documents/User/voice/") {
            if model.zipFilePath.contains((String(str_shareText) + String(str_kindViewTitle.suffix(5)))) {
                //: model.zipFilePath = "\(GestureCacheDefine.getChatVoicePath())\((model.zipFilePath as NSString).lastPathComponent)"
                model.zipFilePath = "\(GestureCacheDefine.sprechstimme())\((model.zipFilePath as NSString).lastPathComponent)"
                //: } else if model.zipFilePath.contains("Documents/User/record/") {
            } else if model.zipFilePath.contains((String(str_touchSourceName.prefix(2)) + str_progressTitle.replacingOccurrences(of: "custom", with: "n") + "s/User" + String(str_randomContent.suffix(8)))) {
                //: model.zipFilePath = "\(GestureCacheDefine.getChatRecordPath())\((model.zipFilePath as NSString).lastPathComponent)"
                model.zipFilePath = "\(GestureCacheDefine.chatGroup())\((model.zipFilePath as NSString).lastPathComponent)"
            }
            ...
            model.taskStatus = .prepare
            //: VoxReactiveCompatible.db_updateVoiceMsg(msgId: model.taskId, status: "\(DownloadTaskStatus.prepare.rawValue)", filePath: model.zipFilePath)
            VoxReactiveCompatible.toAGreaterExtent(msgId: model.taskId, status: "\(ControlFreeWidthNumber.prepare.rawValue)", filePath: model.zipFilePath)
    ...
    func accumulation(_ audioInfo: [String: Any], completeHandle: @escaping OneKeyGreetDownloadBlock) {
        //: var taskModel = VoiceDownloadTaskModel.getOneKeyGreetModel(audioInfo)
        var taskModel = RawTaskModel.get(audioInfo)
        ...
        if self.downloadingModelDic.keys.contains(taskModel.downloadUri) {
            ...
            taskModel = self.downloadingModelDic[taskModel.downloadUri] ?? RawTaskModel()
        }

        // 重编译，相对路径会变
        //: if taskModel.zipFilePath.contains("Documents/User/voice/") {
        if taskModel.zipFilePath.contains((String(str_shareText) + String(str_kindViewTitle.suffix(5)))) {
            //: taskModel.zipFilePath = "\(GestureCacheDefine.getChatVoicePath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            taskModel.zipFilePath = "\(GestureCacheDefine.sprechstimme())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            //: } else if taskModel.zipFilePath.contains("Documents/User/record/") {
        } else if taskModel.zipFilePath.contains((String(str_touchSourceName.prefix(2)) + str_progressTitle.replacingOccurrences(of: "custom", with: "n") + "s/User" + String(str_randomContent.suffix(8)))) {
            //: taskModel.zipFilePath = "\(GestureCacheDefine.getChatRecordPath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            taskModel.zipFilePath = "\(GestureCacheDefine.chatGroup())\((taskModel.zipFilePath as NSString).lastPathComponent)"
        }
...
extension HeadWithDataDelegate: URLSessionDataDelegate {
    // 开始
    ...
    func urlSession(_: URLSession, dataTask: URLSessionDataTask, didReceive _: URLResponse, completionHandler: @escaping (URLSession.ResponseDisposition) -> Void) {
        ...
        let totalBytesWritten = RawTaskModel.getBack(path: taskModel.zipFilePath)
    ...
    func urlSession(_: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        if error != nil || errorCode != 200 {
            ...
            if let error = error, (error as NSError).userInfo[NSLocalizedDescriptionKey] as? String == (String(str_labValue) + str_toBagData.lowercased()) {
                //: if taskModel.taskStatus == .pause {
                if taskModel.taskStatus == .pause {
                    //: pauseTask(url: task.taskDescription ?? "", isSuspend: true)
                    constraintList(url: task.taskDescription ?? "", isSuspend: true)
                    //: } else if taskModel.taskStatus == .cancel {
                } else if taskModel.taskStatus == .cancel {
                    //: self.cancelTask(url: task.taskDescription ?? "")
                    self.equalView(url: task.taskDescription ?? "")
                }
                //: } else {
            } else {
                ...
                constraintList(url: task.taskDescription ?? "", isSuspend: false)
            }
            //: } else {
        } else {
            ...
            self.panVideo(url: task.taskDescription ?? "")
        }
...
extension HeadWithDataDelegate {
    ...
    func constraintList(url: String, isSuspend: Bool) {
        ...
        case .chatRecord:
            ...
            let status = isSuspend ? "\(ControlFreeWidthNumber.pause.rawValue)" : "\(ControlFreeWidthNumber.error.rawValue)"
            //: VoxReactiveCompatible.db_updateVoiceMsg(msgId: taskModel.taskId, status: status)
            VoxReactiveCompatible.toAGreaterExtent(msgId: taskModel.taskId, status: status)
            //: if isSuspend == false {
            if isSuspend == false {
                //: self.delegate?.voiceDownloadError(model: taskModel)
                self.delegate?.voiceInsideComputerSimulation(model: taskModel)
            }
    ...
    func equalView(url: String) {
        ...
        //: case .chatRecord:
        case .chatRecord:
            //: VoxReactiveCompatible.db_deleteVoiceMsg(msgId: taskModel.taskId)
            VoxReactiveCompatible.titleId(msgId: taskModel.taskId)
            //: self.delegate?.voiceDownloadCancel(model: taskModel)
            self.delegate?.voiceShow(model: taskModel)
    ...
    func panVideo(url: String) {
        ...
        let size = RawTaskModel.getBack(path: taskModel.zipFilePath)
        //: if size > 0 {
        if size > 0 {
            //: if taskModel.taskTypeId == .chatRecord {
            if taskModel.taskTypeId == .chatRecord {
                //: VoxReactiveCompatible.db_updateVoiceMsg(msgId: taskModel.taskId, status: "\(DownloadTaskStatus.finish.rawValue)")
                VoxReactiveCompatible.toAGreaterExtent(msgId: taskModel.taskId, status: "\(ControlFreeWidthNumber.finish.rawValue)")
            }

            //: let filePath = "\(taskModel.zipFilePath).wav"
            let filePath = "\(taskModel.zipFilePath)" + (String(str_leadingValueTitle))
            ...
            self.arrangement(at: taskModel.zipFilePath, withKeyId: taskModel.taskId, withTypeId: taskModel.taskTypeId) { _ in
                //: switch taskModel.taskTypeId {
                switch taskModel.taskTypeId {
                //: case .chatRecord:
                case .chatRecord:
                    //: self.delegate?.voiceDownloadFinish(model: taskModel)
                    self.delegate?.isTab(model: taskModel)
            ...
            //: case .chatRecord:
            case .chatRecord:
                //: VoxReactiveCompatible.db_updateVoiceMsg(msgId: taskModel.taskId, status: "\(DownloadTaskStatus.expired.rawValue)")
                VoxReactiveCompatible.toAGreaterExtent(msgId: taskModel.taskId, status: "\(ControlFreeWidthNumber.expired.rawValue)")
                //: self.delegate?.voiceDownloadExpired(model: taskModel)
                self.delegate?.blockBy(model: taskModel)
    ...
    func arrangement(at path: String, withKeyId keyId: String, withTypeId typeId: EqualComparable, complete: @escaping (Error?) -> Void) {
        //: DispatchQueue.global().async {
        DispatchQueue.global().async {
            //: if let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
            if let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
                //: let newPath = "\(path).wav"
                let newPath = "\(path)" + (String(str_leadingValueTitle))
                ...
                if let handle = FileHandle(forWritingAtPath: newPath) {
                    ...
                    if typeId == .chatRecord {
                        //: VoxReactiveCompatible.db_updateVoiceMsg(msgId: keyId, filePath: newPath)
                        VoxReactiveCompatible.toAGreaterExtent(msgId: keyId, filePath: newPath)
                    }
