...
fileprivate let str_managerName:[UInt8] = [0xdc,0xdb,0xdc,0xc1,0x9d,0xd6,0xda,0xd1,0xd0,0xc7,0x8f,0x9c,0x95,0xdd,0xd4,0xc6,0x95,0xdb,0xda,0xc1,0x95,0xd7,0xd0,0xd0,0xdb,0x95,0xdc,0xd8,0xc5,0xd9,0xd0,0xd8,0xd0,0xdb,0xc1,0xd0,0xd1]

private func modelI(layer num: UInt8) -> UInt8 {
    return num ^ 181
}

/*: "Send to %@" :*/
fileprivate let str_collectionName:[Character] = ["S","e"]
fileprivate let str_leadingName:[Character] = ["n","d"," ","t","o"," ","%","@"]

/*: "all" :*/
fileprivate let str_viewErrorData:String = "makel"

/*: "Send to All (Total %@)" :*/
fileprivate let str_minData:String = "Send request frame"
fileprivate let str_sizeName:String = "class tol (Total "
fileprivate let str_clickValue:String = "%@)"

/*: "#FFEE4B" :*/
fileprivate let str_mediumValue:String = "view model model model#FFEE4B"

/*: "\n :*/
fileprivate let str_screenData:String = "\n"

/*: " x :*/
fileprivate let str_shadowName:String = "let aspect back model x"
...
//: @objcMembers public class HeadCellData: TalkingChatMsgBaseCellData {
@objcMembers public class HeadCellData: NameCellData {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_managerName.map{modelI(layer: $0)}, encoding: .utf8)!)
    }
    ...
    public lazy var attributedString: NSMutableAttributedString = {
        ...
        let attr = highGround(giftModel: self.msgModel)
        //: return attr
        return attr
        //: }()
    }()
    ...
    func highGround(giftModel: DiskModel) -> NSMutableAttributedString {
        ...
        let giftname = square(giftmodel: giftModel.gift)
        //: var str = "Send to %@".localizedArguments(giftModel.toUser.nickname)
        var str = (String(str_collectionName) + String(str_leadingName)).localizedArguments(giftModel.toUser.nickname)
        //: if giftModel.toUser.uid == "all" { // 发送所有人礼物, nickname只有数量
        if giftModel.toUser.uid == (str_viewErrorData.replacingOccurrences(of: "make", with: "al")) { // 发送所有人礼物, nickname只有数量
            //: str = "Send to All (Total %@)".localizedArguments(giftModel.toUser.nickname)
            str = (String(str_minData.prefix(5)) + "to Al" + String(str_sizeName.suffix(9)) + str_clickValue.capitalized).localizedArguments(giftModel.toUser.nickname)
        }
        ...
        let color2 = (self.direction == .MsgDirectionOutgoing ? UIColor(hex: (String(str_mediumValue.suffix(7)))) : UIColor.plaster())
    ...
    private func square(giftmodel: AutomaticallyJsonModel) -> String {
