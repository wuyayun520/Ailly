...
/*: "call_bgm" :*/
fileprivate let str_fileDismissValue:[Character] = ["c","a"]
fileprivate let str_listName:String = "ll_bgmin white remove self title"

/*: "call_response_bgm" :*/
fileprivate let str_cellValue:String = "call_rcell camera else"
fileprivate let str_voiceLockValue:[Character] = ["g","m"]
...
class CompartmentPlayListener: NSObject {
    //: var type = CallBGMPlayerType.Call
    var type = DeepRelatableTarget.Call
    //: var player = AVPlayer()
    var player = AVPlayer()
    //: var filePath = ""
    var filePath = ""
    ...
    class func callBy(type: DeepRelatableTarget) -> CompartmentPlayListener {
        //: let player = TalkingCallBGMPlayer()
        let player = CompartmentPlayListener()
        //: player.type = type
        player.type = type
        //: player.getFilePath()
        player.finishApp()
...
extension CompartmentPlayListener {
    //: func play() {
    func appearFor() {
    ...
    private func magnitudeerrupt(notify: Notification) {
    ...
    private func finishApp() {
        ...
        var fileName = (String(str_fileDismissValue) + String(str_listName.prefix(6)))
        //: if self.type == .Response {
        if self.type == .Response {
            //: fileName = "call_response_bgm"
            fileName = (String(str_cellValue.prefix(6)) + "esponse_b" + String(str_voiceLockValue))
        }
        // 收到通话推送要播放mp3声音，不能放到.zip中
        //: let path = SVGAEffectTool.default.getMp3Path(name: fileName)
        let path = ToEffectTool.default.perimeterList(name: fileName)
        //: self.filePath = path
        self.filePath = path
    }
}
...
extension CompartmentPlayListener: TXVodPlayListener {
