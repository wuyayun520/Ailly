...
fileprivate let str_modelToShadowData:[UInt8] = [0xad,0xaa,0xad,0xb0,0xec,0xa7,0xab,0xa0,0xa1,0xb6,0xfe,0xed,0xe4,0xac,0xa5,0xb7,0xe4,0xaa,0xab,0xb0,0xe4,0xa6,0xa1,0xa1,0xaa,0xe4,0xad,0xa9,0xb4,0xa8,0xa1,0xa9,0xa1,0xaa,0xb0,0xa1,0xa0]

private func managerExhibit(make num: UInt8) -> UInt8 {
    return num ^ 196
}
...
class GildTheLilyView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_modelToShadowData.map{managerExhibit(make: $0)}, encoding: .utf8)!)
    }
    ...
    override func willMove(toSuperview _: UIView?) {
        ...
        DeviceSocketDelegate.shared.containerView?.isHidden = false
    }
    ...
    func queryEnd() {
        ...
        UIView.animate(withDuration: 0.3, delay: 2, options: .curveEaseOut) {
            ...
            let x = (self.center.x <= kLet_cameraValue * 0.5) ? -self.width : kLet_cameraValue + self.width
            ...
            DeviceSocketDelegate.shared.containerView?.isHidden = true
        }
    }
}
...
extension GildTheLilyView {
    /// 添加点击手势
    //: @discardableResult
    @discardableResult
    //: func addTapGesture() -> UITapGestureRecognizer {
    func renderSub() -> UITapGestureRecognizer {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(self.tapGestureEvent(sender:)))
        let tap = UITapGestureRecognizer(target: self, action: #selector(self.commit(sender:)))
    ...
    func viewGift() {
        ...
        let pan = UIPanGestureRecognizer(target: self, action: #selector(self.inform(sender:)))
        //: let tap = self.addTapGesture()
        let tap = self.renderSub()
    ...
    @objc func inform(sender: UIPanGestureRecognizer) {
        ...
        centerX = min(kLet_cameraValue - self.width * 0.5, centerX)
        //: centerY = max(self.height*0.5, centerY)
        centerY = max(self.height * 0.5, centerY)
        //: centerY = min(ScreenHeight-self.height*0.5, centerY)
        centerY = min(kLet_clickData - self.height * 0.5, centerY)
        ...
        if sender.state == .ended {
            ...
            var endCenterX = kLet_cameraValue - self.width * 0.5
            //: var recordX = ScreenWidth
            var recordX = kLet_cameraValue
            //: if centerX <= ScreenWidth*0.5 {
            if centerX <= kLet_cameraValue * 0.5 {
            ...
            DeviceSocketDelegate.shared.videoMiniPosition = CGPoint(x: recordX, y: recordY)
        }
    }

    /// 点击手势
    //: @objc private func tapGestureEvent(sender: UIPanGestureRecognizer) {
    @objc private func commit(sender _: UIPanGestureRecognizer) {
