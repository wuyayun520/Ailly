...
fileprivate let str_underDataTitle:[UInt8] = [0xe3,0xe4,0xe3,0xfe,0xa2,0xe9,0xe5,0xee,0xef,0xf8,0xb0,0xa3,0xaa,0xe2,0xeb,0xf9,0xaa,0xe4,0xe5,0xfe,0xaa,0xe8,0xef,0xef,0xe4,0xaa,0xe3,0xe7,0xfa,0xe6,0xef,0xe7,0xef,0xe4,0xfe,0xef,0xee]

private func cellPath(app num: UInt8) -> UInt8 {
    return num ^ 138
}

/*: "%02ld:%02ld:%02ld" :*/
fileprivate let str_whiteName:String = "ret view%02ld:"
fileprivate let str_normalCountMakeValue:String = ":"
fileprivate let str_labelText:String = "return float%02ld"

/*: "icon_mini_voice_end" :*/
fileprivate let str_currentTitle:[UInt8] = [0x64,0x6e,0x65,0x5f,0x65,0x63,0x69,0x6f,0x76,0x5f,0x69,0x6e,0x69,0x6d,0x5f,0x6e,0x6f,0x63,0x69]

/*: "icon_mini_voice_calling" :*/
fileprivate let str_errorName:[Character] = ["i","c","o","n","_","m","i","n","i","_","v","o","i","c"]
fileprivate let str_sizeData:String = "extension super clube_ca"

/*: "#F95151" :*/
fileprivate let str_likeModelName:String = "party return background label#F95151"

/*: "#34C759" :*/
fileprivate let str_headUserTabContent:String = "#34C759name none effect play"

/*: "End of call" :*/
fileprivate let str_frameNowContent:[Character] = ["E","n","d"," ","o","f"," ","c"]
fileprivate let str_pushName:[Character] = ["a","l","l"]
...
class TotalenanceThen: GildTheLilyView {
    // 小窗口通话状态
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_underDataTitle.map{cellPath(app: $0)}, encoding: .utf8)!)
    }
...
extension TotalenanceThen {
    /// 构建音频小窗口
    //: static func buildVoiceMiniView() -> TalkingVoiceMiniView {
    static func cantileverBroadcast() -> TotalenanceThen {
        //: let view = TalkingVoiceMiniView(frame: .zero)
        let view = TotalenanceThen(frame: .zero)
...
extension TotalenanceThen {
    ...
    private func liveConstraint() {
        ...
        lowliness(status: .calling)
    }

    /// 刷新视图
    /// - Parameter status: 通话状态
    //: private func refreshVoiceView(status: MiniWindowStatus) {
    private func lowliness(status: PopWithWindowStatus) {
        //: self.currStatus = status
        self.currStatus = status
        //: let iconStr = (status == .end) ? "icon_mini_voice_end":"icon_mini_voice_calling"
        let iconStr = (status == .end) ? String(bytes: str_currentTitle.reversed(), encoding: .utf8)! : (String(str_errorName) + String(str_sizeData.suffix(4)) + "lling")
        //: self.statusIcon.image = UIImage.eponymUp(name: iconStr)
        self.statusIcon.image = UIImage.eponymUp(name: iconStr)
        //: self.timeLab.textColor = (status == .end) ? UIColor(hex: "#F95151"):UIColor(hex: "#34C759")
        self.timeLab.textColor = (status == .end) ? UIColor(hex: (String(str_likeModelName.suffix(7)))) : UIColor(hex: (String(str_headUserTabContent.prefix(7))))
        //: if status == .end {
        if status == .end {
            //: self.timeLab.text = "End of call".localized
            self.timeLab.text = (String(str_frameNowContent) + String(str_pushName)).localized
        }
    }
}
