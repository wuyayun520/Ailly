...
fileprivate let str_taskFrameData:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
@objc protocol CombineViewDelegate: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func startColor()

    //: @objc optional func endScroll()
    @objc optional func toPin()
}

//: class TalkingVideoDanmuMsgListTableView: UIView {
class PlayDataSource: UIView {
    ...
    override init(frame: CGRect) {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(croquetAction))
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_taskFrameData.reversed(), encoding: .utf8)!)
    }
...
extension PlayDataSource {
    ...
    func dba() {
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(bindComment), userInfo: nil, repeats: true)
    }
    ...
    @objc func bindComment() {
        //: tryToappendAndScrollToBottom()
        stageBottom()
    }

    //: @objc func tapAction() {
    @objc func croquetAction() {
...
extension PlayDataSource {
    ...
    func radicalModelAddVideo(msgModel: WithDanmuModel?) {
    ...
    func cutAcross() {
        ...
        if AllHeight > kLet_conversationMessageDeviceName {
            //: if self.msgTableView.height < VideoMsgTableViewHeight {
            if self.msgTableView.height < kLet_conversationMessageDeviceName {
                ...
                self.msgTableView.height = kLet_conversationMessageDeviceName
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = kLet_conversationMessageDeviceName - AllHeight
        ...
        messageVideo(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func messageVideo(animated: Bool) {
...
extension PlayDataSource: UITableViewDelegate, UITableViewDataSource {
    ...
    func deleteWith(_ scrollView: UIScrollView) {
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(kLet_viewText)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = TargetBassCell.offPath(tableView: tableView, msg: msgModel, indexPath: indexPath)
...
extension PlayDataSource {
    ...
    func humanActionGift() {
        ...
        self.msgTableView.frame = CGRect(x: 0, y: 0, width: kLet_screenName, height: kLet_conversationMessageDeviceName)
    }
}
