...
/*: "id" :*/
fileprivate let str_coinPhotoValue:[Character] = ["i","d"]

/*: "num" :*/
fileprivate let str_alongModeData:String = "titlem"
...
@objc public enum GiftShadowCustomReflectable: Int {
    //: case normal  = 1
    case normal = 1 // 普通礼物
    //: case myStery = 2
    case myStery = 2 // 盲盒礼物
...
struct StatusMeasurable: HandyJSON {
    //: init() {
    init() {}
    ...
    var animationTimes = 0
    //: var showType = 0
    var showType = 0 /// GiftShadowCustomReflectable
    ...
    mutating func startGift(giftNum: Int) {
        //: self.num = giftNum
        self.num = giftNum
    }
}
...
struct ProgressMeasurable: HandyJSON {
    ...
    mutating func mapping(mapper: HelpingMapper) {
        ...
        mapper <<<
            //: self.giftNum <-- "num"
            self.giftNum <-- (str_alongModeData.replacingOccurrences(of: "title", with: "nu"))
    }
}
