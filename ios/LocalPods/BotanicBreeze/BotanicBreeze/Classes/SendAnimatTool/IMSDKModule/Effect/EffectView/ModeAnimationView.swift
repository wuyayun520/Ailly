...
fileprivate let str_videoName:[UInt8] = [0x97,0x90,0x97,0x8a,0xd6,0x9d,0x91,0x9a,0x9b,0x8c,0xc4,0xd7,0xde,0x96,0x9f,0x8d,0xde,0x90,0x91,0x8a,0xde,0x9c,0x9b,0x9b,0x90,0xde,0x97,0x93,0x8e,0x92,0x9b,0x93,0x9b,0x90,0x8a,0x9b,0x9a]

/*: "%@.mp4" :*/
fileprivate let str_reportValue:[Character] = ["%","@",".","m","p"]
fileprivate let str_labData:String = "4"

/*: "http" :*/
fileprivate let str_mainViewImageValue:String = "HTTP"
...
class ModeAnimationView: LengthView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_videoName.map{$0^254}, encoding: .utf8)!)
    }
    ...
    public lazy var vapPlayerView: UIView? = {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(imageActionTap))
    ...
    override func hold() {
        ...
        let path = WithShadowAnimatTool.shared.fileVideo(fileName: self.giftEffectModel?.mainFile ?? "", model: self.effectMsgModel ?? CellModelHandyJSON())

        //: if  path.count > 0 {
        if path.count > 0 {
            //: let mp4Path = String.init(format: "%@.mp4", path)
            let mp4Path = String(format: (String(str_reportValue) + str_labData.capitalized), path)
            ...
            self.delegate?.thanGesture(effectItemView: self)
        }
    }

    //: override func stopAnimating() {
    override func imageAnimating() {
        //: self.vapPlayerView?.stopHWDMP4()
        self.vapPlayerView?.stopHWDMP4()
    }

    //: override func cleanAnimating() {
    override func languageUnit() {
    ...
    @objc func imageActionTap() {}
}

///  --- HWDMP4PlayDelegate

//: extension TalkingVapAnimationView: HWDMP4PlayDelegate {
extension ModeAnimationView: HWDMP4PlayDelegate {
    ...
    func loadVapImage(withURL urlStr: String!, context _: [AnyHashable: Any]!, completion completionBlock: VAPImageCompletionBlock!) {
        ...
        DispatchQueue.main.async {
            ...
            if urlStr.hasPrefix((str_mainViewImageValue.lowercased())) {
