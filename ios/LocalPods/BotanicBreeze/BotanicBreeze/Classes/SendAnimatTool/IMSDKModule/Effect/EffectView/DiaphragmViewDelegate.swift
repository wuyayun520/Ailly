...
fileprivate let str_sizeName:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "iosSvgaEffect" :*/
fileprivate let str_kitValue:[Character] = ["i","o","s","S","v","g","a","E","f","f"]
fileprivate let str_withName:[Character] = ["e","c","t"]

/*: "iosEffect" :*/
fileprivate let str_selectContent:String = "iosEflist make string detail"

/*: "entityId" :*/
fileprivate let str_callData:[Character] = ["e","n","t","i","t","y"]
fileprivate let str_toData:[Character] = ["I","d"]
...
class DiaphragmViewDelegate: UIView {
    //: var currentEffectMsgModel: TalkingAnimatMsgModel?
    var currentEffectMsgModel: CellModelHandyJSON?
    ...
    var animatedView: LengthView?

    //: open weak var delegate: AnimatViewStepProtocol?
    open weak var delegate: SkinStepProtocol?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_sizeName.reversed(), encoding: .utf8)!)
    }
...
extension DiaphragmViewDelegate {
    //: func playAnimation(infoDic: Dictionary<String, Any>) {
    func clickFormat(infoDic: [String: Any]) {
        //: let dict = infoDic
        let dict = infoDic
        //: if dict.keys.isEmpty == false {
        if dict.keys.isEmpty == false {
            //: var iosEffect: String = dict["iosSvgaEffect"] as! String
            var iosEffect: String = dict[(String(str_kitValue) + String(str_withName))] as! String
            //: if iosEffect.isEmptyString {
            if iosEffect.isEmptyString {
                //: iosEffect = dict["iosEffect"] as! String
                iosEffect = dict[(String(str_selectContent.prefix(5)) + "fect")] as! String
            }

            //: let entityId = dict["entityId"] as! Int
            let entityId = dict[(String(str_callData) + String(str_toData))] as! Int
            ...
            if iosEffect.isEmptyString == false {
                ...
                let successs = gatefold(msgModel: effectMsgModel)
            }
        }
    }
    ...
    func gatefold(msgModel: Any?) -> Bool {
        ...
        if msgModel is CellModelHandyJSON {
            //: effectMsgModel = msgModel as? TalkingAnimatMsgModel
            effectMsgModel = msgModel as? CellModelHandyJSON
            //: } else if msgModel is TalkingGiftMsgAnimatModel {
        } else if msgModel is StatusMeasurable {
            //: let giftMsgModel = msgModel as! TalkingGiftMsgAnimatModel
            let giftMsgModel = msgModel as! StatusMeasurable
            ...
            if giftMsgModel.animationTimes > 1 {
                ...
                tmpGiftMsgModel.startGift(giftNum: 1) // 礼物个数设置为1，每次动效显示
                ...
                var repeatEfftect = CellModelHandyJSON()
            ...
            effectMsgModel = CellModelHandyJSON()
            ...
            if giftMsgModel.showType == GiftShadowCustomReflectable.myStery.rawValue || giftMsgModel.showType == GiftShadowCustomReflectable.king.rawValue {
        ...
        if self.currentEffectMsgModel != nil || paused {
            ...
            if effectMsgModel?.top ?? false && self.effectMsgModelArray.count > 0 {
                ...
                for i in 0 ..< self.effectMsgModelArray.count {
                    ...
                    let aModel: CellModelHandyJSON = self.effectMsgModelArray[i] as! CellModelHandyJSON
                    //: if aModel.top == false {
                    if aModel.top == false {
                        //: index = i
                        index = i
                ...
                if index == -1 {
                    ...
                    self.effectMsgModelArray.add(effectMsgModel ?? CellModelHandyJSON())
                    //: } else {
                } else {
                    //: self.effectMsgModelArray.insert(effectMsgModel ?? TalkingAnimatMsgModel.init(), at: index)
                    self.effectMsgModelArray.insert(effectMsgModel ?? CellModelHandyJSON(), at: index)
                }
            ...
            context(effectMsgModel: effectMsgModel!)
        }
...
extension DiaphragmViewDelegate {
    ...
    func precedent() {
        ...
        if self.currentEffectMsgModel == nil {
            ...
            if self.effectMsgModelArray.count > 0 {
                ...
                guard let removeModel = dismantle() else { return }
                //: firstShow(effectMsgModel: removeModel)
                context(effectMsgModel: removeModel)
            }
            //: } else {
        } else {
            //: self.animatedView?.resumeAnimation()
            self.animatedView?.tapRow()
        }
    ...
    func dismantle() -> CellModelHandyJSON? {
        //: if (self.effectMsgModelArray.count == 0) {
        if self.effectMsgModelArray.count == 0 {
            //: return TalkingAnimatMsgModel.init()
            return CellModelHandyJSON()
        }
        ...
        if effectMsgModel is CellModelHandyJSON {
            //: let lastEffectMsgModel = effectMsgModel as! TalkingAnimatMsgModel
            let lastEffectMsgModel = effectMsgModel as! CellModelHandyJSON
        ...
        return effectMsgModel as? CellModelHandyJSON
    }
}
...
extension DiaphragmViewDelegate {
    ...
    func context(effectMsgModel: CellModelHandyJSON) {
        ...
        self.delegate?.refuseObj(effectView: self, obj: effectMsgModel)

        //: TalkingGiftAnimatTool.shared.loadEffectData(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
        WithShadowAnimatTool.shared.dataLocalMatch(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
            ...
            self.audienceAdd(effectMsgModel: effectMsgModel, success: success, effectModel: giftModel ?? UpToTransformable())
            //: })
        })
    }
    ...
    func audienceAdd(effectMsgModel: CellModelHandyJSON, success: Bool, effectModel: UpToTransformable) {
        // 资源下载完成
        //: self.delegate?.animatViewWithDidEndLoadingResource(effectView: self, obj: effectMsgModel, success: success)
        self.delegate?.achromaticColor(effectView: self, obj: effectMsgModel, success: success)
        ...
        if success {
            ...
            if self.paused {
                ...
                if self.currentEffectMsgModel?.eid == effectMsgModel.eid, self.currentEffectMsgModel?.effectUrl == effectMsgModel.effectUrl {
                    ...
                    framework(effectMsgModel: effectMsgModel, model: effectModel)
                    //: return
                    return
                }
            }
    ...
    func withModel(effectMsgModel: CellModelHandyJSON, effectModel: UpToTransformable) {
        //: if self.animatedView != nil {
        if self.animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.imageAnimating()
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.languageUnit()
        ...
        className = WithShadowAnimatTool.shared.taskIntervaleraction(oldName: className ?? "")
        ...
        let realClass = AnimatedViewClass as! LengthView.Type
...
extension DiaphragmViewDelegate: UpViewDelegate {
    ...
    func futurism(effectItemView: LengthView, success: Bool) {
        ...
        self.delegate?.assay(effectView: self, obj: effectItemView.effectMsgModel ?? CellModelHandyJSON(), success: success)
    }
    ...
    func thanGesture(effectItemView: LengthView) {
        ...
        self.delegate?.phone(effectView: self, obj: effectMsgModel!)
    }
}
...
extension DiaphragmViewDelegate {
    ...
    // clean
    //: func cleanData() {
    func cleanAggregation() {
        //: if animatedView != nil {
        if animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.imageAnimating() // stop animation
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.languageUnit() // clean animation data
