...
fileprivate let str_shareContent:[UInt8] = [0x19,0x1e,0x19,0x24,0xd8,0x13,0x1f,0x14,0x15,0x22,0xea,0xd9,0xd0,0x18,0x11,0x23,0xd0,0x1e,0x1f,0x24,0xd0,0x12,0x15,0x15,0x1e,0xd0,0x19,0x1d,0x20,0x1c,0x15,0x1d,0x15,0x1e,0x24,0x15,0x14]

fileprivate func chemicalModel(rank num: UInt8) -> UInt8 {
...
/*: "Online Users" :*/
fileprivate let str_infoMakeData:String = "Online label cell mini value"
fileprivate let str_normalValue:[Character] = ["U","s","e","r","s"]

/*: "UpViewCell" :*/
fileprivate let str_atText:String = "TalkingLsituation equal deadline view window"
fileprivate let str_pinTimeData:String = "if send bagiveR"
fileprivate let str_modelData:String = "LINE"
fileprivate let str_tableContent:String = "right let effectCell"
...
class FileView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_shareContent.map{chemicalModel(rank: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var dismissBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(changeBuild), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var tipsLb: UILabel = {
        ...
        lb.text = (String(str_infoMakeData.prefix(7)) + String(str_normalValue)).localized
        //: return lb
        return lb
        //: }()
    }()
...
extension FileView {
    ...
    private func messageData() {
        ...
        SurgeryReqTool.nameSlate(roomId: ViewRoomManager.astatine().partyModel.roomId) { succeed, result, _ in
            ...
            if let datas = [HeadTransformable].deserialize(from: array as? Array) {
...
extension FileView {
    ...
    @objc private func changeBuild() {
        //: dismiss()
        presentationAcrossDismiss()
    }
    ...
    func beforeView() {
        ...
        contentView.frame = CGRect(x: 0, y: kLet_clickData, width: kLet_cameraValue, height: contentV_H)
        //: reqData()
        messageData()
    ...
    /// 隐藏视图
    //: func dismiss() {
    func presentationAcrossDismiss() {
        //: UIView.animate(withDuration: 0.3) {
        UIView.animate(withDuration: 0.3) {
            //: self.contentView.frame.origin.y += self.contentView.height
            self.contentView.frame.origin.y += self.contentView.height
...
extension FileView: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let identifier = (String(str_atText.prefix(8)) + String(str_pinTimeData.suffix(4)) + "oomOn" + str_modelData.lowercased() + "List" + String(str_tableContent.suffix(4)))
        ...
        cell?.tantamountStart(model: DataSource[indexPath.row], index: indexPath.row)
        //: return cell!
        return cell!
    }
