...
/*: "Search" :*/
fileprivate let str_userData:[Character] = ["S","e","a","r","c","h"]

/*: "#F5F5F5" :*/
fileprivate let str_cellText:String = "#F5F5F5model var item play"

/*: "Search by UID" :*/
fileprivate let str_emptyLocalName:String = "Searcjump to voice in view"
fileprivate let str_tableData:String = "d"

/*: "icon_search_sea" :*/
fileprivate let str_cornerData:String = "share case let hide leticon_sea"
fileprivate let str_logValue:[Character] = ["r","c","h","_","s","e","a"]

/*: "Can’t find~" :*/
fileprivate let str_imageTopContent:String = "Can\u{2019}t "

/*: "icon_kong_kong_default" :*/
fileprivate let str_freeValue:[Character] = ["i","c","o","n","_","k","o","n","g","_","k","o","n"]
fileprivate let str_redText:String = "name return label conversation managerg_default"

/*: "Please enter the query UID" :*/
fileprivate let str_aboutName:String = "Pleainfo user end value range"
fileprivate let str_pointSubContent:String = "er thsub let view margin at"
fileprivate let str_tabData:String = "live else returnery UI"
fileprivate let str_selectExamineionContent:String = "d"
...
class MakeRecognizerDelegate: SeamViewController {
    ...
    private lazy var searchBtn: UIButton = {
        ...
        btn.setTitle((String(str_userData)).localized, for: .normal)
        ...
        btn.setBackgroundImage(UIImage.makeBackground(colors: UIColor.appToVoice(), size: CGSize(width: 72, height: 32)), for: .normal)
    ...
    private lazy var searchTF: UITextField = {
        ...
        tf.backgroundColor = UIColor(hex: (String(str_cellText.prefix(7))))
        //: tf.textColor = UIColor.appTitleColor()
        tf.textColor = UIColor.executeTo()
        //: tf.font = UIFont.outsideAllSize(fontSize: 16)
        tf.font = UIFont.outsideAllSize(fontSize: 16)
        ...
        tf.placeholder = (String(str_emptyLocalName.prefix(5)) + "h by UI" + str_tableData.uppercased()).localized
    ...
    private lazy var unlessEditingView: UIView = {
        //: let image = UIImage.eponymUp(name: "icon_search_sea")
        let image = UIImage.eponymUp(name: (String(str_cornerData.suffix(8)) + String(str_logValue)))
    ...
    private lazy var tableView: UITableView = {
        ...
        tabV.register(InstallmentItemCell.self, forCellReuseIdentifier: InstallmentItemCell.className())
    ...
    private lazy var emptyV: EmptyView = {
        ...
        style.TipsTitle = (str_imageTopContent + "find~").localized
        //: style.TipsIcon = "icon_kong_kong_default"
        style.TipsIcon = (String(str_freeValue) + String(str_redText.suffix(9)))
        ...
        let v = EmptyView(frame: CGRect(x: 0, y: 14 + kLet_deviceRecordValue, width: kLet_cameraValue, height: kLet_clickData - (14 + kLet_deviceRecordValue + kLet_detailNameText)), style: style)
    ...
    private lazy var reqManager: ShadowReactiveCompatible = //: return ShadowReactiveCompatible()
        .init()
    //: }()
}
...
extension MakeRecognizerDelegate {
    ...
    private func loadTo() {
        ...
        guard searchTF.text?.count ?? 0 > 0 else {
            ...
            self.oversight(showMsg: (String(str_aboutName.prefix(4)) + "se ent" + String(str_pointSubContent.prefix(5)) + "e qu" + String(str_tabData.suffix(6)) + str_selectExamineionContent.uppercased()).localized)
            //: return
            return
        }
        ...
        reqManager.singleRoomEqual(uid: searchTF.text ?? "") { [weak self] _, _, _ in
            //: ProgressHUD.dismiss()
...
extension MakeRecognizerDelegate: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: InstallmentItemCell.className(), for: indexPath) as! InstallmentItemCell
        //: let model = reqManager.searchListDataArr[indexPath.row]
        let model = reqManager.searchListDataArr[indexPath.row]
        //: cell.refreshCell(model: model)
        cell.restoreModel(model: model)
...
extension MakeRecognizerDelegate: UITextFieldDelegate {
...
extension MakeRecognizerDelegate: JXPagingViewListViewDelegate {
...
extension MakeRecognizerDelegate {
    ...
    private func confinementWithView() {
