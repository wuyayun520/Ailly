...
class ToReactiveCompatible: NSObject {
    ...
    var config = PointOfReferenceReactiveCompatible()          // 视频渲染配置类
    ...
    private var trtcView: RawViewDelegete?    // 预览view（使用商汤美颜）
...
extension ToReactiveCompatible {
    ...
    func doingTotal(_ frontCamera: Bool) {
    ...
    func outsideHandle(_ openCamera: Bool) {
    ...
    func changeBlock() {
        ...
        DispatchQueue.main.async {
            ...
            self.replyUp(smallWindow: self.infoModel?.renderPostion != .selfOnBg)
...
extension ToReactiveCompatible {
    ...
    private func sideFront(front: Bool) {
        ...
        self.trtcView?.appearUp(isFront: front)
    }
    
    /// 是否开启摄像头
    /// - Parameter isOpen: 是否开启
    //: private func setTRTCCamera(isOpen: Bool) {
    private func rowTo(isOpen: Bool) {
        //: guard SenseTime_Use == true else {
        guard kLet_infoData == true else {
            //: if isOpen {
            if isOpen {
        ...
        self.trtcView?.eraseOff(isOpen: isOpen)
...
extension ToReactiveCompatible {
    ...
    func totalerrupt() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(modelEqual(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(imageAfter(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(statusImageNotif(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(perceiver(notif:)),
    ...
    @objc private func modelEqual(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
            // 静音
    ...
    @objc private func imageAfter(notif: Notification?) {
        //: if notif == nil { 
    ...
    @objc private func statusImageNotif(notif: Notification) {
    ...
    @objc func perceiver(notif: Notification) {
...
extension ToReactiveCompatible {
    ...
    func replyUp(smallWindow: Bool) {
        ...
        if self.trtcView == nil {
            ...
            self.trtcView = RawViewDelegete()
        }
        //: self.trtcView?.startPreView(isSmallWindow: smallWindow)
        self.trtcView?.ofImage(isSmallWindow: smallWindow)
        //: if smallWindow { 
