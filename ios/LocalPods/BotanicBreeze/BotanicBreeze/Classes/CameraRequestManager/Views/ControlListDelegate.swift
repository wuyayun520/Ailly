...
fileprivate let str_successTitle:[UInt8] = [0xf8,0xff,0xf8,0xe5,0xb9,0xf2,0xfe,0xf5,0xf4,0xe3,0xab,0xb8,0xb1,0xf9,0xf0,0xe2,0xb1,0xff,0xfe,0xe5,0xb1,0xf3,0xf4,0xf4,0xff,0xb1,0xf8,0xfc,0xe1,0xfd,0xf4,0xfc,0xf4,0xff,0xe5,0xf4,0xf5]

/*: "Not yet~" :*/
fileprivate let str_listScreenValue:String = "Not color push"

/*: "#8C7AFF" :*/
fileprivate let str_afterFileTitle:[Character] = ["#","8","C","7","A","F","F"]
...
class ControlListDelegate: UIView {
    //: var type = 0
    var type = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_successTitle.map{$0^145}, encoding: .utf8)!)
    }
    ...
    lazy var dataArray: [MTheoryTransformable] = [] {
        //: didSet {
        didSet {
    ...
    lazy var ranktableView = UITableView(frame: self.bounds, style: .plain).then {
        ...
        $0.register(UnrestrainedReactiveCompatible.self, forCellReuseIdentifier: UnrestrainedReactiveCompatible.className())
        //: $0.register(SocialRankListViewCell.self, forCellReuseIdentifier: SocialRankListViewCell.className())
        $0.register(OfViewCell.self, forCellReuseIdentifier: OfViewCell.className())
    }
    ...
    lazy var tableHeaderView = RecaptureReactiveCompatible(frame: CGRect(x: 0, y: 0, width: kLet_cameraValue, height: actualHeight(h: 220 + 14))).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.index = self.type
        $0.index = self.type
    ...
    lazy var tableBackgrourdView = TargetEmptyView().then {
        //: $0.backgroundColor = .white
        $0.backgroundColor = .white
        //: $0.desLab.text = "Not yet~".localized
        $0.desLab.text = (String(str_listScreenValue.prefix(4)) + "yet~").localized
        //: $0.desLab.textColor = UIColor(hex: "#8C7AFF")
        $0.desLab.textColor = UIColor(hex: (String(str_afterFileTitle)))
...
extension ControlListDelegate: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: UnrestrainedReactiveCompatible.className(), for: indexPath) as! UnrestrainedReactiveCompatible
            //: return cell
            return cell
        }
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: OfViewCell.className(), for: indexPath) as! OfViewCell
        //: let model = dataArray[indexPath.row+3]
        let model = dataArray[indexPath.row + 3]
        //: cell.updateRankCell(model: model, index: indexPath.row+3)
        cell.bowling(model: model, index: indexPath.row + 3)
        //: cell.updateIcon(type: self.type)
        cell.temper(type: self.type)
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        if indexPath.section == 0 {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: UnrestrainedReactiveCompatible.className()) as! UnrestrainedReactiveCompatible
...
extension ControlListDelegate: JXSegmentedListContainerViewListDelegate {
