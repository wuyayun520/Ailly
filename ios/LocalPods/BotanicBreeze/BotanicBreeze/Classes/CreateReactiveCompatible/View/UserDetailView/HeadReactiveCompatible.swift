...
protocol CompeteLayoutDelegate: NSObjectProtocol {
...
class HeadReactiveCompatible: UICollectionViewFlowLayout {
    //: var delegate: TalkingTagCollectionFlowLayoutDelegate?
    var delegate: CompeteLayoutDelegate?
    ...
    override func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
        ...
        for i in 1..<attributes.count {
            ...
            if i == attributes.count-1 {
                ...
                self.delegate?.tillSuccess(layoutH: curAttr.frame.maxY)
            }
        }
