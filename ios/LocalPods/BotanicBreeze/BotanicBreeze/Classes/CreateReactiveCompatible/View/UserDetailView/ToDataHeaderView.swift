...
fileprivate let str_sectionName:[UInt8] = [0x33,0x34,0x33,0x2e,0x72,0x39,0x35,0x3e,0x3f,0x28,0x60,0x73,0x7a,0x32,0x3b,0x29,0x7a,0x34,0x35,0x2e,0x7a,0x38,0x3f,0x3f,0x34,0x7a,0x33,0x37,0x2a,0x36,0x3f,0x37,0x3f,0x34,0x2e,0x3f,0x3e]

/*: "icon_userdetail_header_shadow" :*/
fileprivate let str_iconRoomValue:String = "in var tap lab allicon_u"
fileprivate let str_nameText:String = "ETAI"
fileprivate let str_holdText:String = "right make add selfder_s"
fileprivate let str_blockSucceedUpTitle:[Character] = ["h","a","d","o","w"]

/*: "icon_lounge" :*/
fileprivate let str_managerData:String = "hidden ificon_l"

/*: "Official" :*/
fileprivate let str_textData:String = "Offigift view center to title"

/*: "Reviewing" :*/
fileprivate let str_managerValue:String = "Reviewinmake my manager player make"
fileprivate let str_viewData:[Character] = ["g"]

/*: "#FF506D" :*/
fileprivate let str_portValue:String = "#FF506error import true"
fileprivate let str_picValue:String = "d"

/*: "icon_user_detail_intimate_header" :*/
fileprivate let str_beContent:[UInt8] = [0x64,0x6e,0x62,0x63,0x52,0x78,0x7e,0x68,0x7f,0x52,0x69,0x68,0x79,0x6c,0x64,0x61,0x52,0x64,0x63,0x79,0x64,0x60,0x6c,0x79,0x68,0x52,0x65,0x68,0x6c,0x69,0x68,0x7f]

private func decisionMaker(count num: UInt8) -> UInt8 {
    return num ^ 13
}

/*: "?toUid= :*/
fileprivate let str_adjustValue:String = "?toUifalse view top var let"
fileprivate let str_normalContent:String = "clear background equal selfd="

/*: "OccurrentReactiveCompatible" :*/
fileprivate let str_coordinateData:String = "view burn select dismissTUse"
fileprivate let str_sourceData:String = "ilHeadsuper voice table"
fileprivate let str_barData:String = "gift report to callmCell"

/*: "#FFFFFF" :*/
fileprivate let str_viewFrameName:[Character] = ["#","F","F","F","F","F","F"]

/*: "#BDB8B5" :*/
fileprivate let str_thenValue:[Character] = ["#","B","D","B","8","B","5"]

/*: "head_detail_bottom" :*/
fileprivate let str_indicatorName:String = "true in cancel female pinhead_"
fileprivate let str_backgroundName:String = "l_bottomplayer direction view extension"

/*: "#2DF2FF" :*/
fileprivate let str_equalText:String = "#2DF2FFreturn case to info return"

/*: "Live" :*/
fileprivate let str_adjustData:[Character] = ["L","i","v","e"]

/*: "#48FF1E" :*/
fileprivate let str_centerTitle:[Character] = ["#"]
fileprivate let str_sizeMapName:String = "model self data self48FF1E"

/*: "Online" :*/
fileprivate let str_inputName:[Character] = ["O","n","l","i","n"]
fileprivate let str_imageName:[Character] = ["e"]

/*: "#FF4B7F" :*/
fileprivate let str_buttonName:String = "#Fuser guard"
fileprivate let str_withName:String = "Fcurrent7F"

/*: "busy" :*/
fileprivate let str_giftName:String = "buswill"
...
let kLet_deviceData = (302.0 + kLet_timeSenseValue)

//: class TalkingUserDetailHeaderView: UIView {
class ToDataHeaderView: UIView {
    //: var VideoCallBlock: (() -> Void)?
    var VideoCallBlock: (() -> Void)?
    //: private var userInfoModel = TalkingUserInfoModel()
    private var userInfoModel = OccurrentInfoModel()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_sectionName.map{$0^90}, encoding: .utf8)!)
    }
    ...
    private lazy var shadowBgView: UIImageView = {
        //: let v = UIImageView(image: UIImage.eponymUp(name: "icon_userdetail_header_shadow"))
        let v = UIImageView(image: UIImage.eponymUp(name: (String(str_iconRoomValue.suffix(6)) + "serd" + str_nameText.lowercased() + "l_hea" + String(str_holdText.suffix(5)) + String(str_blockSucceedUpTitle))))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var vipBgView: UIImageView = {
        ...
        v.frame = CGRect(x: 0, y: kLet_deviceData - 15, width: kLet_cameraValue, height: 15)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.eponymUp(name: (String(str_managerData.suffix(6)) + "ounge"))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var officalImgV: UIButton = {
        ...
        img.setTitle((String(str_textData.prefix(4)) + "cial").localized, for: .normal)
    ...
    private lazy var onlineImgV: ProgressTalkingButton = {
    ...
    private lazy var liveView: ListMiniView = {
        ...
        v.tapGestureBlock = { [weak self] in
            ...
            guard let self = self else { return }
            //: EditTextThen.share.func_audiencePushToLiveRoomVC(uid: self.userInfoModel.uid, enterType: .userDetail)
            EditTextThen.share.studyShow(uid: self.userInfoModel.uid, enterType: .userDetail)
        }
        //: return v
        return v
        //: }()
    ...
    private lazy var reViewingLB: UILabel = {
        ...
        lb.text = (String(str_managerValue.prefix(8)) + String(str_viewData)).localized
        //: lb.textColor = .white
        lb.textColor = .white
        //: lb.backgroundColor = UIColor.init(hex: "#FF506D")
        lb.backgroundColor = UIColor(hex: (String(str_portValue.prefix(6)) + str_picValue.uppercased()))
    ...
    lazy var lastLoginLab: UILabel = {
        ...
        lb.font = .pingfang(type: .Medium, fontSize: 12)
        //: lb.textColor = .white
        lb.textColor = .white
        //: return lb
        return lb
    ...
    private lazy var intimateBtn: ProgressTalkingButton = {
        //: let btn = TalkingButton()
        let btn = ProgressTalkingButton()
        //: btn.setImage(UIImage.eponymUp(name: "icon_user_detail_intimate_header"), for: .normal)
        btn.setImage(UIImage.eponymUp(name: String(bytes: str_beContent.map{decisionMaker(count: $0)}, encoding: .utf8)!), for: .normal)
        //: btn.isHidden = true
        btn.isHidden = true
        //: btn.addTarget(self, action: #selector(intimateBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(underInsert), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension ToDataHeaderView {
    /// 查看视频
    //: private func goToVideo() {
    private func editDetail() {
        //: let vc = TalkingMomentVideoVC.init(videoPath: self.userInfoModel.videoUrl)
        let vc = AtViewController(videoPath: self.userInfoModel.videoUrl)
        //: if EqualThen.share.loginUid != self.userInfoModel.uid {
        if EqualThen.share.loginUid != self.userInfoModel.uid {
    ...
    private func secret(_ index: Int) {
        ...
        let vc = TargetPhotosVc(momentModel: currenModel, index: index, type: .normal)
        //: self.currentViewController()?.navigationController?.pushViewController(vc, animated: true)
        self.infoController()?.navigationController?.pushViewController(vc, animated: true)
    }

    //: @objc func intimateBtnClick() {
    @objc func underInsert() {
        //: let url = TalkingWebManager.getFullUrl(urlType: .Intimate) + "?toUid=\(self.userInfoModel.uid)"
        let url = SendManager.duringTap(urlType: .Intimate) + (String(str_adjustValue.prefix(5)) + String(str_normalContent.suffix(2))) + "\(self.userInfoModel.uid)"
...
extension ToDataHeaderView: JXBannerDataSource {
    ...
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        ...
        return JXBannerCellRegister(type: OccurrentReactiveCompatible.self, reuseIdentifier: (String(str_coordinateData.suffix(4)) + "rDeta" + String(str_sourceData.prefix(6)) + "erIte" + String(str_barData.suffix(5))))
    }
    ...
    func jxBanner(_: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
        ...
        if index == 0 && userInfoModel.sex == RunReplaceableCollection.female.rawValue && userInfoModel.videoUrl.count > 1 { // 视频
            ...
            bannerCell.titleUrl(videoUrl: userInfoModel.videoUrl, imgUrl: userInfoModel.cover)
            ...
            bannerCell.bgImgView.lineBorder(urlStr: urlStr, placeImg: UIImage.banner())
        }
    ...
    func jxBanner(_: JXBannerType, centerIndex: Int, centerCell _: UICollectionViewCell) {
        //: guard userInfoModel.uid == EqualThen.share.loginUid else {
        guard userInfoModel.uid == EqualThen.share.loginUid else {
            //: return
            return
        }
    ...
    func jxBanner(pageControl _: JXBannerType, numberOfPages _: Int, coverView: UIView, builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder {
        ...
        pageControl.activeColor = UIColor(hex: (String(str_viewFrameName)))!
        //: pageControl.inactiveColor = UIColor(hex: "#BDB8B5")!
        pageControl.inactiveColor = UIColor(hex: (String(str_thenValue)))!
...
extension ToDataHeaderView: JXBannerDelegate {
    ...
    public func jxBanner(_: JXBannerType, didSelectItemAt index: Int) {
        //: if index == 0 && userInfoModel.sex == Gender.female.rawValue && userInfoModel.videoUrl.count>1 {
        if index == 0 && userInfoModel.sex == RunReplaceableCollection.female.rawValue && userInfoModel.videoUrl.count > 1 {
            //: goToVideo()
            editDetail()
            ...
            if userInfoModel.sex == RunReplaceableCollection.female.rawValue && userInfoModel.videoUrl.count > 1 && curren > 0 {
                //: curren -= 1
                curren -= 1
            }
            //: goToPhoto(curren)
...
extension ToDataHeaderView {
    ...
    func joint(_ userModel: OccurrentInfoModel) {
        ...
        if userModel.sex == RunReplaceableCollection.female.rawValue && userModel.videoUrl.count > 1 {
        ...
        bannerView.reloadView()
        // vip
        //: let normalImg = UIImage.eponymUp(name: "head_detail_bottom").withTintColor(.white)
        let normalImg = UIImage.eponymUp(name: (String(str_indicatorName.suffix(5)) + "detai" + String(str_backgroundName.prefix(8)))).withTintColor(.white)
        //: if userModel.loungePlus {
        if userModel.loungePlus {
            //: if let vipModel = EqualThen.share.appUserConfigMode.vipSetting.filter({$0.vipSkinId == userModel.vipSkinId}).first {
            if let vipModel = EqualThen.share.appUserConfigMode.vipSetting.filter({ $0.vipSkinId == userModel.vipSkinId }).first {
                //: vipBgView.setUrlImage(urlStr: vipModel.vipSkinTop, placeImg: normalImg)
                vipBgView.lineBorder(urlStr: vipModel.vipSkinTop, placeImg: normalImg)
            }
            //: } else {
        } else {
        ...
        self.nicknameLab.textColor = userModel.loungePlus ? .active() : .white
...
//        }
        ...
        if userModel.livePlayUrl.count > 0 {
            ...
            let image = UIImage.impressionDataSize(color: UIColor(hex: (String(str_equalText.prefix(7))))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.circle(), for: .normal)
            onlineImgV.setImage(image.circleWithoutImagination(), for: .normal)
            //: onlineImgV.setTitle("Live".localized, for: .normal)
            onlineImgV.setTitle((String(str_adjustData)).localized, for: .normal)
            ...
            let image = UIImage.impressionDataSize(color: UIColor(hex: (String(str_centerTitle) + String(str_sizeMapName.suffix(6))))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.circle(), for: .normal)
            onlineImgV.setImage(image.circleWithoutImagination(), for: .normal)
            //: onlineImgV.setTitle("Online".localized, for: .normal)
            onlineImgV.setTitle((String(str_inputName) + String(str_imageName)).localized, for: .normal)
            ...
            let image = UIImage.impressionDataSize(color: UIColor(hex: (String(str_buttonName.prefix(2)) + str_withName.replacingOccurrences(of: "current", with: "4B")))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.circle(), for: .normal)
            onlineImgV.setImage(image.circleWithoutImagination(), for: .normal)
            //: onlineImgV.setTitle("busy".localized, for: .normal)
            onlineImgV.setTitle((str_giftName.replacingOccurrences(of: "will", with: "y")).localized, for: .normal)
            //: } else {
        } else {
        ...
        // 直播
        //: if EqualThen.share.loginUid != userModel.uid &&
        if EqualThen.share.loginUid != userModel.uid,
        ...
        if liveView.isHidden == false {
            ...
            liveView.objectSearch(liveUrl: userModel.livePlayUrl)
        }
    }

    /// 添加视图
    //: private func setupSubviews() {
    private func landStatus() {
    ...
    private func keep() {
        ...
        liveView.snp.makeConstraints { make in
            ...
            make.top.equalTo(kLet_timeSenseValue + 80)
        ...
        reViewingLB.snp.makeConstraints { make in
            ...
            make.top.equalToSuperview().offset(kLet_timeSenseValue + 15)
