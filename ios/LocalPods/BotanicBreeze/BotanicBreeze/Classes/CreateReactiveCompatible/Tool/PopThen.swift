...
/*: "detection" :*/
fileprivate let str_spaceCallEqualName:[Character] = ["d"]
fileprivate let str_centerName:String = "ETECTION"

/*: "aiff" :*/
fileprivate let str_freeTitle:[Character] = ["a","i","f","f"]
...
class PopThen: NSObject {
    // singleton
    //: static let shared = RBDMuteSwitch()
    static let shared = PopThen()
    ...
    func supercedeMake() {
        ...
        let soundFilePath = ToEffectTool.default.perimeterList(name: (String(str_spaceCallEqualName) + str_centerName.lowercased()), type: (String(str_freeTitle)))
    ...
    @objc private func incrementButton() {
        //: soundDuration += 0.001
        soundDuration += 0.001
    }
