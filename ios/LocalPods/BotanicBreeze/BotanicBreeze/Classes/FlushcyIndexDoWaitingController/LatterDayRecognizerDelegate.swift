...
fileprivate let str_toTitle:[UInt8] = [0x55,0x52,0x55,0x48,0x14,0x5f,0x53,0x58,0x59,0x4e,0x6,0x15,0x1c,0x54,0x5d,0x4f,0x1c,0x52,0x53,0x48,0x1c,0x5e,0x59,0x59,0x52,0x1c,0x55,0x51,0x4c,0x50,0x59,0x51,0x59,0x52,0x48,0x59,0x58]

private func dataMental(model num: UInt8) -> UInt8 {
    return num ^ 60
}
...
// __CLOSE_PRINT__
//
//  LatterDayRecognizerDelegate.swift
//  AbroadTalking
//
...
class LatterDayRecognizerDelegate: SeamViewController {
    //: var tranConfig: BasePresentTransition!
    var tranConfig: ExistingPresentTransition!
    //: init() {
    init() {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: self.tranConfig = BasePresentTransition.init(target: self)
        self.tranConfig = ExistingPresentTransition(target: self)
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: str_toTitle.map{dataMental(model: $0)}, encoding: .utf8)!)
    }
}

//: class BasePresentTransition: NSObject {
class ExistingPresentTransition: NSObject {
    ...
    var frameOfPresentedView = CGRect(x: 0, y: 0, width: kLet_cameraValue, height: kLet_clickData * 0.6)
    ...
    func atRadius(corners: UIRectCorner, radius: CGFloat) {
...
extension ExistingPresentTransition: UIViewControllerTransitioningDelegate {
    ...
    func presentationController(forPresented presented: UIViewController, presenting: UIViewController?, source _: UIViewController) -> UIPresentationController? {
        ...
        let res = GestureRecognizerDelegate(transition: self, presentedViewController: presented, presentingViewController: presenting)
        //: return res
        return res
    }
}
...
extension ExistingPresentTransition: UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if toVC == targetVC {
            ...
            fossilisation(transitionContext: transitionContext)
            //: } else {
        } else {
            //: dismissTransition(transitionContext: transitionContext)
            exceptFinish(transitionContext: transitionContext)
        }
    }

    //: private func presentTransition(transitionContext: UIViewControllerContextTransitioning) {
    private func fossilisation(transitionContext: UIViewControllerContextTransitioning) {
        //: let toVC = transitionContext.viewController(forKey: .to)
        let toVC = transitionContext.viewController(forKey: .to)
    ...
    private func exceptFinish(transitionContext: UIViewControllerContextTransitioning) {
...
class GestureRecognizerDelegate: UIPresentationController, UIGestureRecognizerDelegate {
    //: var transition: BasePresentTransition!
    var transition: ExistingPresentTransition!

    //: init(transition: BasePresentTransition, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    init(transition: ExistingPresentTransition, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    ...
    func gildTheLily() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(lift))
        //: tap.delegate = self
        tap.delegate = self
    ...
    lazy var visualView: UIView = {
        ...
        view.frame = containerView?.bounds ?? CGRect(x: 0, y: 0, width: kLet_cameraValue, height: kLet_clickData)
