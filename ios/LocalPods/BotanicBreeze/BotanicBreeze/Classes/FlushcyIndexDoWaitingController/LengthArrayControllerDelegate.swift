...
// __CLOSE_PRINT__
//
//  LengthArrayControllerDelegate.swift
//  AbroadTalking
//
...
class LengthArrayControllerDelegate: UINavigationController {

    //: var tabBarType: TabBarItemType?
    var tabBarType: DeepEquatable?
    //: var pushing = false                             
    ...
    override func pushViewController(_ viewController: UIViewController, animated: Bool) {
        ...
        if viewControllers.count >= 1 {
            ...
            if viewController.navigationItem.leftBarButtonItem == nil {
                ...
                viewController.navigationItem.leftBarButtonItem = viewController.pathView()
            }
        }
    ...
    func titleAppearance() {
        ...
        if #available(iOS 15.0, *) {
            ...
            barApp.backgroundImage = UIImage.noMain(color: UIColor.white)
            //: barApp.shadowImage = UIImage.init()
            barApp.shadowImage = UIImage.init()
            //: barApp.shadowColor = UIColor.appBgColor()
            barApp.shadowColor = UIColor.imageColor()
            ...
            barApp.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.executeTo(), NSAttributedString.Key.font: UIFont.pingfang(type: .Medium, fontSize: 17)]
        ...
        //: } else {
            ...
            navigationBar.setBackgroundImage(UIImage.noMain(color: UIColor.white), for: .any, barMetrics: .default)
            ...
            navigationBar.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.executeTo(), NSAttributedString.Key.font: UIFont.pingfang(type: .Medium, fontSize: 17)]
        }
        //: UINavigationBar.appearance().tintColor = UIColor.appTitleColor()
        UINavigationBar.appearance().tintColor = UIColor.executeTo()
...
extension LengthArrayControllerDelegate: UINavigationControllerDelegate {
...
extension LengthArrayControllerDelegate: UIGestureRecognizerDelegate {
