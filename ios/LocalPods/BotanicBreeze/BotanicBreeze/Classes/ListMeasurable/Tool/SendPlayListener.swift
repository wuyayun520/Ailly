...
/*: "/TXCache" :*/
fileprivate let str_dateValue:[Character] = ["/","T"]
fileprivate let str_mName:String = "XCacheimage case"
...
protocol CameraReactiveCompatible: NSObject {
    //: func func__playerStatus(player: TalkingVideoPlayerManager, status: VideoPlayerStatus)
    func titleCell(player: SendPlayListener, status: TitleerPlayerStatus)

    //: func func__playerDuration(player: TalkingVideoPlayerManager, duration: Int, currentTime: Int)
    func playTable(player: SendPlayListener, duration: Int, currentTime: Int)

    //: func func__playerProgress(player: TalkingVideoPlayerManager, progress: CGFloat)
    func dataScript(player: SendPlayListener, progress: CGFloat)
}

//: class TalkingVideoPlayerManager: NSObject {
class SendPlayListener: NSObject {
    //: var isPlaying = false
    var isPlaying = false
    //: var isMute = false
    var isMute = false
    ...
    open weak var delegate: CameraReactiveCompatible?
    //: var playStatus: VideoPlayerStatus?
    var playStatus: TitleerPlayerStatus?
    //: static let shared = TalkingVideoPlayerManager()
    static let shared = SendPlayListener()
    ...
    lazy var player: TXVodPlayer = {
        ...
        let cachePath = NSString(format: "%@%@", NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first!, (String(str_dateValue) + String(str_mName.prefix(6)))) as String
...
extension SendPlayListener {
    //: func playerWithUrlAndVideoView(url: String, view: UIView) {
    func requestView(url: String, view: UIView) {
        //: let newUrl = url.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let newUrl = url.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: kLet_deviceName)
    ...
    @objc func run() {
        ...
        if duration() > 0 {
            ...
            self.delegate?.dataScript(player: self, progress: CGFloat(currentTime() / duration()))
        }
    }

    //: func setRenderMode(renderMode: EnumTypeRenderMode) {
    func giftMode(renderMode: UserEnumRenderMode) {
    ...
    func wreak(bEnable: Bool) {
        //: var isMute = bEnable
        var isMute = bEnable
        //: if TalkingSocketManager.shared.isTalking == true {
        if DeviceSocketDelegate.shared.isTalking == true { // 音视频通话时静音
            //: isMute = true
            isMute = true
        }
    ...
    func showLock() {
        ...
        if self.playStatus != .Pausing {
            ...
            self.delegate?.titleCell(player: self, status: self.playStatus!)
        }
    }
...
extension SendPlayListener: TXVodPlayListener {
    ...
    func onPlayEvent(_ player: TXVodPlayer!, event EvtID: Int32, withParam _: [AnyHashable: Any]!) {
        ...
        case PLAY_EVT_PLAY_PROGRESS.rawValue:
            ...
            self.delegate?.playTable(player: self, duration: Int(duration), currentTime: Int(currentTime))
