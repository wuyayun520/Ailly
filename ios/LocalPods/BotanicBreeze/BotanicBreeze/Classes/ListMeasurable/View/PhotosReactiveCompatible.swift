...
fileprivate let str_formatValue:[UInt8] = [0x1f,0x18,0x1f,0x2,0x5e,0x15,0x19,0x12,0x13,0x4,0x4c,0x5f,0x56,0x1e,0x17,0x5,0x56,0x18,0x19,0x2,0x56,0x14,0x13,0x13,0x18,0x56,0x1f,0x1b,0x6,0x1a,0x13,0x1b,0x13,0x18,0x2,0x13,0x12]

private func toPhoto(card num: UInt8) -> UInt8 {
    return num ^ 118
}
...
class PhotosReactiveCompatible: UICollectionViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_formatValue.map{toPhoto(card: $0)}, encoding: .utf8)!)
    }

    //: func customUI() {
    func liveTo() {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(imageViewClick))
        let tap = UITapGestureRecognizer(target: self, action: #selector(cease))
        ...
        let doubletap = UITapGestureRecognizer(target: self, action: #selector(atTap))
...
extension PhotosReactiveCompatible: UIScrollViewDelegate {
    //: @objc func imageViewClick(tap: UIGestureRecognizer) {
    @objc func cease(tap _: UIGestureRecognizer) {
        //: if sblock != nil {
        if sblock != nil {
            //: sblock()
            sblock()
    ...
    @objc func atTap(tap: UIGestureRecognizer) {
    ...
    func titleTo(url: String) {
        //: imageView.setUrlImage(urlStr: url)
        imageView.direct(urlStr: url)
    }

    //: func configImage(image: UIImage) {
    func overSubImage(image: UIImage) {
        //: imageView.image = image
        imageView.image = image
    }

    //: func refreshImageContainerViewCenter () {
    func innerCitySelected() {
