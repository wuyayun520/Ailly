...
fileprivate let str_controlText:[UInt8] = [0x2f,0x28,0x2f,0x32,0x6e,0x25,0x29,0x22,0x23,0x34,0x7c,0x6f,0x66,0x2e,0x27,0x35,0x66,0x28,0x29,0x32,0x66,0x24,0x23,0x23,0x28,0x66,0x2f,0x2b,0x36,0x2a,0x23,0x2b,0x23,0x28,0x32,0x23,0x22]

private func makeClear(position num: UInt8) -> UInt8 {
    return num ^ 70
}
...
class PanoplyPlayerSlider: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_controlText.map{makeClear(position: $0)}, encoding: .utf8)!)
    }
    ...
    init(frame: CGRect, sliderWidth: CGFloat, sliderColor: UIColor, progressHeight: CGFloat, progressBgColor: UIColor, progressPlayedColor: UIColor, progressCachedColor: UIColor, isShowCorner: Bool) {
        ...
        self.swaddlingClothesDataSubEmpty()
        //: self.bindInteraction()
        self.moveCounteraction()
    }
    ...
    private lazy var progressView: TextMakeView = {
        ...
        let v = TextMakeView(frame: self.frame, progressBgColor: self.progressBgColor, progressPlayedColor: self.progressPlayedColor, progressCachedColor: self.progressCachedColor)
    ...
    private lazy var panGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(humanActivity(gesture:)))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var tapGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(gestureCutJabbing(gesture:)))
        //: return v
        return v
        //: }()
    }()
...
extension PanoplyPlayerSlider {
    //: @objc func panGesture(gesture: UIPanGestureRecognizer) {
    @objc func humanActivity(gesture: UIPanGestureRecognizer) {
        // 移动的距离
    ...
    @objc func gestureCutJabbing(gesture: UIPanGestureRecognizer) {
...
extension PanoplyPlayerSlider {
    ...
    private func swaddlingClothesDataSubEmpty() {
    ...
    private func moveCounteraction() {}
}
