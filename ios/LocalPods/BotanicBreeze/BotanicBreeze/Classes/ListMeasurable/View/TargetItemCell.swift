...
fileprivate let str_fileShowName:[UInt8] = [0x4b,0x50,0x4b,0x56,0xa,0x45,0x51,0x46,0x47,0x54,0x1c,0xb,0x2,0x4a,0x43,0x55,0x2,0x50,0x51,0x56,0x2,0x44,0x47,0x47,0x50,0x2,0x4b,0x4f,0x52,0x4e,0x47,0x4f,0x47,0x50,0x56,0x47,0x46]

fileprivate func kitLab(head num: UInt8) -> UInt8 {
...
/*: "#4FAAFF" :*/
fileprivate let str_listName:String = "#"
fileprivate let str_normalName:String = "i var4FAAFF"

/*: "reply" :*/
fileprivate let str_timeData:String = "rviewly"

/*: "： :*/
fileprivate let str_pageData:[Character] = ["："]

/*: "F5F5F5" :*/
fileprivate let str_layerInfoSucceedValue:String = "fieldfieldfield"
...
class TargetItemCell: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_fileShowName.map{kitLab(head: $0)}, encoding: .utf8)!)
    }
...
extension TargetItemCell {
    //: @objc func onLongTapCell(sender: UIGestureRecognizer) {
    @objc func accept(sender _: UIGestureRecognizer) {}
}

//: extension TalkingCommentReplyItemCell {
extension TargetItemCell {
    //: func configCell(model: TalkingCommentReplyModel) {
    func equalModel(model: ListMeasurable) {
        //: if model.allCountContent.count>0 {
        if model.allCountContent.count > 0 {
            //: contentLB.textColor = UIColor.init(hex: "#4FAAFF")
            contentLB.textColor = UIColor(hex: (str_listName.capitalized + String(str_normalName.suffix(6))))
            ...
            //: var fontA = [UIFont]()
            var fontA = [UIFont]()
            //: let font = UIFont.outsideAllSize(fontSize: 14)
            let font = UIFont.outsideAllSize(fontSize: 14)
            //: if model.replyUser.count>0 {
            if model.replyUser.count > 0 {
                //: strA = [model.nickname, " "+"reply".localized+" ", model.replyUser, "：\(model.content)"]
                strA = [model.nickname, " " + (str_timeData.replacingOccurrences(of: "view", with: "ep")).localized + " ", model.replyUser, "：\(model.content)"]
                //: colorA = [UIColor.publishView(), UIColor.appTitleColor(), UIColor.publishView(), UIColor.appTitleColor()]
                colorA = [UIColor.publishView(), UIColor.executeTo(), UIColor.publishView(), UIColor.executeTo()]
                ...
                strA = [model.nickname, "：\(model.content)"]
                //: colorA = [UIColor.publishView(), UIColor.appTitleColor()]
                colorA = [UIColor.publishView(), UIColor.executeTo()]
                //: fontA = [font, font]
                fontA = [font, font]
            }
    ...
    func toRoom(cell: UITableViewCell, tableView: UITableView, indexPath: NSIndexPath, radius: CGFloat, height: CGFloat) {
        ...
        let bounds = CGRect(x: 51, y: 0, width: kLet_cameraValue - 51 - 15, height: cell.bounds.size.height)
        ...
        layer.fillColor = UIColor(hex: (str_layerInfoSucceedValue.replacingOccurrences(of: "field", with: "F5")))?.cgColor
...
extension TargetItemCell {
    ...
    private func pagination() {
        ...
        let longtap = UILongPressGestureRecognizer(target: self, action: #selector(accept(sender:)))
