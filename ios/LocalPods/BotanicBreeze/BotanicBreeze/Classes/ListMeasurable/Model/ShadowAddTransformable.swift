...
/*: "id" :*/
fileprivate let str_leadingText:String = "ishow"

/*: "reply" :*/
fileprivate let str_fileText:String = "repllist"

/*: :  :*/
fileprivate let str_observerTitle:[Character] = [":"," "]

/*:  : :*/
fileprivate let str_roomTitle:String = " :"
...
struct ShadowAddTransformable: HandyJSON {
    //: var cid: String?
...
extension ShadowAddTransformable {
    ...
    mutating func label() {
        ...
        if replyUser?.count ?? 0 > 0 {
            ...
            if FileReactiveCompatible.shared.direction == .leftToRight {
                ...
                tempContent = (str_fileText.replacingOccurrences(of: "list", with: "y")).localized + "\(replyUser!): \(content!)"
                //: } else {
            } else {
                //: tempContent = "\(content!) :\(replyUser!)" + "reply".localized
                tempContent = "\(content!) :\(replyUser!)" + (str_fileText.replacingOccurrences(of: "list", with: "y")).localized
            }
        ...
        let sizeheight = Int(AddDataReactiveCompatible.coequal(text: tempContent, size: CGSize(width: kLet_cameraValue - 70, height: CGFloat(MAXFLOAT)), font: UIFont.pingfang(type: .Regular, fontSize: 16), lineNumber: 0).height)
