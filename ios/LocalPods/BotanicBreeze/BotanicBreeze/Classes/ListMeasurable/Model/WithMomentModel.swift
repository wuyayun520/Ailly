...
/*: "id" :*/
fileprivate let str_backgroundValue:String = "userd"
...
struct WithMomentModel: HandyJSON {
    //: var uid: String?
    var uid: String?
    //: var sex: String?
    var sex: String?
...
enum NameOfUTF8Initializable: String {
    //: case nones  = "0"
    case nones = "0"
    //: case Photo = "1"
    case Photo = "1"
...
struct GestureHandyJSON: HandyJSON {
    //: var videoUrl = ""
    var videoUrl = ""
    //: var imgUrl = ""
    var imgUrl = ""
    ...
    var status: DoingSequence?
}

///
//: enum MomentVideoStatus: String {
enum DoingSequence: String {
    //: case NoPass = "-1"   // 未通过
    case NoPass = "-1" // 未通过
    //: case Audit = "0"     // 审核中
    case Audit = "0" // 审核中
...
extension WithMomentModel {
    //: mutating func caculateItemHeight(isPost: Bool = false) {
    mutating func income(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            textHeight = Int(AddDataReactiveCompatible.coequal(text: content!, size: CGSize(width: kLet_cameraValue - 54, height: CGFloat(MAXFLOAT)), font: UIFont.pingfang(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }

        //: if momentType == MomentType.Photo.rawValue || momentType == MomentType.Live.rawValue {
        if momentType == NameOfUTF8Initializable.Photo.rawValue || momentType == NameOfUTF8Initializable.Live.rawValue {
            //: if pic.count==1 {
            if pic.count == 1 {
                //: photesHeight = 212
                photesHeight = 212
