...
/*: "id" :*/
fileprivate let str_withActualName:[Character] = ["i","d"]

/*: "%@ reply %@：%@" :*/
fileprivate let str_buttonTitle:[Character] = ["%","@"," ","r","e","p"]
fileprivate let str_callName:String = "ly %@：%@let load scale model wrap"

/*: " %@：%@" :*/
fileprivate let str_targetData:String = " %method：%method"
...
struct ListMeasurable: HandyJSON {
    //: var cid: String?
...
extension ListMeasurable {
    ...
    mutating func keyOf() {
        ...
        if replyUser.count > 0 {
            ...
            tempContent = String(format: (String(str_buttonTitle) + String(str_callName.prefix(8))), nickname, replyUser, content)
            //: } else {
        } else {
        ...
        let sizeheight = Int(AddDataReactiveCompatible.coequal(text: tempContent, size: CGSize(width: kLet_cameraValue - 82, height: CGFloat(MAXFLOAT)), font: UIFont.pingfang(type: .Regular, fontSize: 14), lineNumber: 0).height)
