...
fileprivate let str_statusData:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "img_home_shadow" :*/
fileprivate let str_existLeadingName:String = "let size data selfimg_"
fileprivate let str_willContent:String = "_"
fileprivate let str_actionValue:[Character] = ["s","h","a","d","o","w"]

/*: "icon_match_dislikeBtn_nor" :*/
fileprivate let str_shareValue:String = "ilabn"
fileprivate let str_tagDataName:String = "ch_disin self center a true"
fileprivate let str_rangeValue:String = "height restore age toolBtn_n"
fileprivate let str_pathName:String = "size"

/*: "icon_match_likeBtn_nor" :*/
fileprivate let str_viewValue:String = "icon_player true lab price view"
fileprivate let str_ageValue:[Character] = ["i","k","e","B"]
fileprivate let str_ofData:String = "tn_norparty data"
...
protocol JogObjectProtocol: NSObjectProtocol {
    ///
    //: func numberOfCountInCardView(cardView: TalkingClubCardView) -> Int
    func ofImport(cardView: MakeCardView) -> Int
...
@objc protocol LengthTalkingViewDelegate: NSObjectProtocol {
    ///
    ...
    @objc optional func toGesture(cardView: MakeCardView, cardViewCell: ClubThen, forRowAtIndex: Int, direction: Int, isGesture: Bool)

    //: @objc optional func didRemoveLastCell(cardViewCell: TalkingClubCardViewCell, forRowAtIndex: Int)
    @objc optional func decisionMaking(cardViewCell: ClubThen, forRowAtIndex: Int)

    //: @objc optional func didDisplayCell(cardViewCell: TalkingClubCardViewCell, forRowAtIndex: Int)
    @objc optional func imageIndex(cardViewCell: ClubThen, forRowAtIndex: Int)

    //: @objc optional func didMoveCell(cardView: TalkingClubCardView, cardViewCell: TalkingClubCardViewCell, forMovePoint: CGPoint)
    @objc optional func workingPapers(cardView: MakeCardView, cardViewCell: ClubThen, forMovePoint: CGPoint)

    //: @objc optional func beginMoveCell(cardView: TalkingClubCardView, cardViewCell: TalkingClubCardViewCell)
    @objc optional func beginLikeCompartment(cardView: MakeCardView, cardViewCell: ClubThen)
}

//: class TalkingClubCardView: UIView {
class MakeCardView: UIView {
    ...
    var maxRemoveDistance = kLet_cameraValue / 4.0 /// 最大移除距离(默认屏幕的1/4)
    //: var reusableCells = [TalkingClubCardViewCell]()
    var reusableCells = [ClubThen]() /// 重用卡片数组
    //: var isAlpha = true
    ...
    var dataSource: JogObjectProtocol? /// 数据源
    //: var delegate: TalkingClubCardViewDelegate?
    var delegate: LengthTalkingViewDelegate?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_statusData.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var leftbackImageView: UIImageView = {
        ...
        imag.contentMode = .scaleAspectFill
        //: imag.image = UIImage.eponymUp(name: "img_home_shadow")
        imag.image = UIImage.eponymUp(name: (String(str_existLeadingName.suffix(4)) + "home" + str_willContent.capitalized + String(str_actionValue)))
    ...
    private lazy var disLikeImage: UIImageView = {
        //: let img = UIImageView()
        let img = UIImageView()
        //: img.image = UIImage.eponymUp(name: "icon_match_dislikeBtn_nor")
        img.image = UIImage.eponymUp(name: (str_shareValue.replacingOccurrences(of: "lab", with: "co") + "_mat" + String(str_tagDataName.prefix(6)) + "like" + String(str_rangeValue.suffix(5)) + str_pathName.replacingOccurrences(of: "size", with: "or")))
        //: img.isHidden = true
        img.isHidden = true
        //: img.alpha = 0.5
        img.alpha = 0.5
    ...
    private lazy var likeImage: UIImageView = {
        //: let img = UIImageView()
        let img = UIImageView()
        //: img.image = UIImage.eponymUp(name: "icon_match_likeBtn_nor")
        img.image = UIImage.eponymUp(name: (String(str_viewValue.prefix(5)) + "match_l" + String(str_ageValue) + String(str_ofData.prefix(6))))
        //: img.isHidden = true
        img.isHidden = true
        //: img.alpha = 0.5
        img.alpha = 0.5
...
extension MakeCardView {
    //: func reloadDataAnimated(anim: Bool) {
    func sharedCurrent(anim: Bool) {
        //: moveCount = 0
        moveCount = 0
        //: self.currentIndex = 0
        self.currentIndex = 0
        ...
        let maxCount = dataSource?.ofImport(cardView: self) ?? 0
    ...
    func viewDetail(index: Int) {
        //: let cell = dataSource?.cardViewCellForRowAtIndex(cardView: self, index: index)
        let cell = dataSource?.dismissOn(cardView: self, index: index)
...
//        let showCount = Double(self.visibleCount - 1)
        ...
        containerView.insertSubview(cell ?? ClubThen(), at: 0)
    ...
    func upAnim(anim: Bool) {
        ...
        for i in 0 ... count - 1 {
            ...
            let cell: ClubThen = self.visibleCells[i] as! ClubThen
            ...
            if isVisbleLast {
                ...
                self.delegate?.imageIndex?(cardViewCell: cell, forRowAtIndex: self.currentIndex - i)
            }

            //: if anim {
            if anim {
                //: updateConstraintsCell(cell: cell, transform: transform)
                errorMin(cell: cell, transform: transform)
                //: } else {
            } else {
    ...
    func errorMin(cell: ClubThen, transform: CGAffineTransform) {
    ...
    func splitModel(cellClass: AnyClass, forellIdentifier: String) {
    ...
    func per(identifier: String) -> ClubThen? {
        ...
        if self.cellClass != nil {
            ...
            let cellclass = self.cellClass as! BottomAgendaItemReactiveCompatible.Type
            //: let cell: TalkingClubCardItemCell = cellclass.init(reuseIdentifier: identifier )
            let cell: BottomAgendaItemReactiveCompatible = cellclass.init(reuseIdentifier: identifier)
...
extension MakeCardView: StudCellDelegete {
    ...
    func flaunt(cell: ClubThen, direction: DataTradingCardCustomReflectable, isGesture: Bool) {
        ...
        self.delegate?.toGesture?(cardView: self, cardViewCell: cell, forRowAtIndex: self.currentFirstIndex - 1, direction: direction.rawValue, isGesture: isGesture)
        ...
        if self.visibleCells.count == 0 { // 数据看完了
            ...
            self.delegate?.decisionMaking?(cardViewCell: cell, forRowAtIndex: self.currentIndex)
            //: return
            return
        }

        //: let count = self.dataSource?.numberOfCountInCardView(cardView: self) ?? 0
        let count = self.dataSource?.ofImport(cardView: self) ?? 0

        // 当前数据源还有数据 继续创建cell
        //: if (self.currentIndex < count - 1) {
        if self.currentIndex < count - 1 {
            //: createCardViewCell(index: self.currentIndex + 1)
            viewDetail(index: self.currentIndex + 1)
        }
        // 更新布局
    ...
    func unwelcome(cell: ClubThen, forMovePoint: CGPoint, direction: DataTradingCardCustomReflectable) {
        ...
        self.delegate?.workingPapers?(cardView: self, cardViewCell: cell, forMovePoint: forMovePoint)
        //: if  direction == .Left{
        if direction == .Left {
    ...
    func cardFromCompartment(cell _: ClubThen) {
        //: disLikeImage.isHidden = true
        disLikeImage.isHidden = true
        //: likeImage.isHidden = true
        likeImage.isHidden = true
    ...
    func paperAdd(cell: ClubThen) {
        ...
        self.delegate?.beginLikeCompartment?(cardView: self, cardViewCell: cell)
    }
}
